...
| 臨床表現(clinical_manifestations)英譯 | 195 列 zh-only(Public EN 模式「症狀表現」段 196 張卡印中文散文),10 批譯 + 覆核 workflow;覆核 9 個 + 第 10 批譯者被 429 打掉 → 先落 166 列:機械段數比對(132 等長、34 列差 1–5 段全是英文單一症狀內含逗號)+ 主線逐列人眼讀,field_sources 註明「獨立覆核待補跑」 | both 0 / zhOnly 195 → 166 / 29 | opt-0909e(CI 後落) |
| 現代對應病名 + 7 方 symptoms(修正 + 資料) | formulaModernDiseaseSection 只讀 zh → english 改 en‖zh;97 方 1,301 個病名 = 219 個唯一名做字典(4 批譯 + 覆核:222 ok、4 採覆核修正,含 3 個原文疑錯字「尿道膿傷/女性乳房腫廇」與相容字「重症肌無力」U+F98A)→ 逐方逐項對映(NFKC 比對鍵);symptoms_en 7 方 111 項逐索引 | modern_diseases both 0/97 → 97/0;symptoms 193/7 → 200/0 | 9ffd2b45 |
...
| 穴位清單延遲渲染(包 C 候選 A,凍結例外) | Opus 實作於 `claude/pkgC-cards-lazy`(render() 的 renderCards 改成進 acu 才畫第一次;hash 自己判定 + activeWs + fail-open)。三鏡頭對抗審查(導航 20 路徑 / 閘門 15 種語意破壞 / 真瀏覽器 16 路徑)抓到 2 MED:M1 開機期 settle 的 `readyState==="loading"` 分支是死碼,實際走 setTimeout(0),router.js 晚到時 fail-open 首頁全畫 947 張(三鏡頭各自實測 + 我的 defer-test 重現:timer 1 ms、下一支 defer 1214 ms)→ 改成 complete 才 setTimeout、否則等 DOMContentLoaded;M2 閘門純形狀比對 14/15 破壞仍綠 → 觸發點要「活的」+ 五個守門函式抽出來跑行為真值表,負控 11 → 21 | 修後實測:router.js 延遲 4 s 首頁 0 張 / 3,121 節點、進 acu 947 / 16,888、#point/LI4 947 + 合谷、router.js 404 時 fail-open 947 / 16 section 全開;正常伺服器開機 0 / 3,121 | 4252fd53 |

Public EN 模式「還印中文」分類(兩個唯讀 agent,量在 data/generated):方劑卡 17 區塊裡,渲染器「中英並印 / 硬編碼中文、不看 contentMode」佔絕大多數 —— `detailPairedList`(682–724)一支管功用/主治/加減/研究/應用/禁忌六區塊,改一次少 ~44,500 字、零翻譯;現代運用索引(38,644)、藥對 pairCard(32,774)、組成表(~21,000)同型;真要翻譯才會消失的只有 CloudTCM 深度三欄 notes/fang_yi/zhu_zhi 51 筆 21,902 字(翻譯 workflow 已發)與藥對 teaching_note 42 筆。中藥卡 16 區塊同病:pairCard/慎用/禁忌/主治/功效/藥理 chip/病名標籤/劑量與來源硬字全無分支,改渲染器可少 ~128k 字;`modern_functions_detail`(46 萬字 / 192 味,CloudTCM 散文)渲染器 items 只讀 analysis_zh(有 en 陣列的 3 味整段消失)—— 先修一行,要不要翻 46 萬字待 Ting;慎用/禁忌 19+20 味 zh/en 長度不等卻逐索引配對(英文可能配到錯的句子)→ 加 aligned 護欄。兩個假缺口:八法 95 卡的「缺 en」是佔位句中文化後穿過守門;學習備註 61 卡有 58 卡是順天堂顆粒服法樣板。中藥卡渲染器批已派實作(`claude/herb-en-mode`)。
...
| 批 | 內容 | 結果 |
|---|---|---|
| 第三批(中文回正) | 病症卡針灸實證的結果指標/研究發現/穴位角色 47、腎經注意事項 11、中藥古籍註 1 | encoding 107 → 48 |
| 第四批(英文補齊) | 方劑症狀 193/219、八法、注意事項 149/219;中藥炮製 105/360、現代藥理 78/360 | 3,093 條 |
| 量測修正 | 連結品牌名、西藥學名、評分表欄位名、schema 定義檔 | encoding 156 → 107 |

**卡面確認**:小青龍湯在英文模式的「症狀表現」整區從空白變成完整英文;
臺灣藥典那格印「有收載(臺灣中藥典 p.129)」。防風、瀉白散前一批已確認過。

## 兩個自己抓到的坑(都寫進自測了)

1. **陣列中間被否決 → 半套錯位**。第三批有 50 條被覆核否決、63 條 agent 自己跳過,
   若只填前綴,第 3 條中文會配到第 4 條英文 —— 比整格空白危險得多。
   改成「同一欄位要嘛全填、要嘛不填」,因此丟掉 97 條。實測:這批造成的中英不對齊 **0** 筆
   (既有 29 筆是舊帳,另案)。
2. **落庫工具把整檔重排版**。`apply-field-ledger` 的縮排偵測抓「第一個縮排的引號」,
   在裸陣列檔(meridian_ki.json)上抓到第二層 → 2 格變 4 格,語意只改 11 條、diff 卻 1,385 行,
   沒有人能在那種 diff 上覆核。改成取最小縮排單位;自測補上這個形狀(11 條)。

## 誠實留下的

encoding 還有 48 筆,其中 38 筆是方劑的音譯不確定詞(Loffler 氏症候群這類),agent 拒譯、我沒有替它猜。
`herb_canon` 5,495 與 `herb_track_filler` 1,131 兩層都沒動 —— 那要等 Ting 對 C6 的裁定。

# 2026-09-02 夜 — 卡片內容線:2,675 條落庫(中文回正 1,460 + 英文補齊 1,215);外加一個會誘發造假的閘門被擋住

D1 回滾之後 Ting 說「先優化其他部分」。今晚做的都是**搬運型**工作 —— 來源已經在同一張卡上,
只是擺錯欄位或缺另一語言。沒有一條是新查來的內容。

## 落庫的數字

| 批 | 內容 | 前 → 後 |
|---|---|---|
| 中文回正 | 英文寫在 `_zh` 欄位(方劑 modern_applications / treats / 臺灣藥典、中藥 modern_pharmacology) | encoding 缺陷 **1,456 → 156** |
| 英文補齊 | 中藥卡歸經 / 適應症 / 禁忌 | 歸經 81→351、適應症 126→355、禁忌 156→359(共 360 張在庫卡) |
| 假陽性清掉 | `diagram_urls_zh` 裝的是網址不是中文散文 | encoding −361(只豁免「整個值就是一個 URL」) |
| ICD 主碼 | crosswalk 第 0 位改成 canon `icd_hint` 指的選項 | disagrees 2 → 0 |
| 退役卡 | H5 雙語對齊不再套用在已撤下的青木香上 | herb_card_schema 1 → 0 |

**卡面已用眼睛確認**:瀉白散的現代運用索引現在印「玫瑰痤瘡 · Rosacea」「慢性阻塞性肺病(COPD)」;
防風的適應症在英文模式印出完整英文。以前那兩格中文版讀者看到的是英文、英文版讀者看到的是空白。

## 方法:帳本模式(以後派工都照這個)

agent **唯讀**產出 `{file, record, path, expect, value}` 帳本 → `scripts/apply-field-ledger.js` 逐條比對
「現況是否仍等於 expect」才寫。24 個 Sonnet agent 分 12 批翻譯 + 12 批獨立覆核,兩批合計:
覆核 reject 3、覆核改寫 43、agent 自己 skip 36(音譯不確定的 Loffler 氏症候群、疑似 OCR 損壞的「表尖」都拒譯,誠實留白)。
落庫 2,675 條、現況不符 **0**、錯誤 **0**;中英索引長度 0 不對齊。

## 擋下一個會誘發造假的閘門

`herb_canon` 那 5,495 個缺陷,壓倒性多數是「200 張中藥卡缺 `english_exam_track` 的 7 個必填欄位」。查下去:

- `js/knowledge.js` 的 `herbPanels` 宣告了 `const exam = record.english_exam_track || {}`,**底下一個 exam 欄位都沒用** —— 那些必填欄位不會出現在任何卡面。
- 已經有的 204 張裡 **181 張裝的是樣板句**:「Review pregnancy review before clinical use.」一字不差出現在 110 張卡;麻黃與桂枝的 functions 完全相同。
- 也就是說把 5,495 補綠最省力的做法,就是照同一個模子再生 162 份 —— 閘門全綠、卡面零改變、真假從此分不出來。

對策不是刪閘門(那是 Ting 的裁定,已寫成待裁 C6),而是加一層讓那條路走不通:
`scripts/validate-herb-track-filler.js` + 棘輪層 `herb_track_filler`(基準 1,131,只准降)。
第一版對中文欄位也判「逐字重複」,把「發汗解表」這種真術語算成樣板(2,424 條假陽性)——
限縮成 english_exam_track 的英文長句(≥5 詞)後,chinese_depth_track 負控 0 條。

## 新工具

- `scripts/report-card-slot-gaps.js` —— **依渲染器實際讀什麼**量卡面空格,不是依欄位空不空。
  `A || B` 二選一要整條鏈都空才算空格;「宣告了卻沒用」的欄位標成「補了也看不到」。
  判「沒用到」踩了兩次坑:註解裡的 `exam`、CSS 類名 `k-exam-badge` 各造成一次假陰性,現在先去註解再用識別字位置比對。
- `scripts/apply-field-ledger.js` —— 帳本落庫(10 條自測:現況不符拒寫、跳號拒寫、帳本自我矛盾整批拒絕、dry-run 不寫檔、縮排沿用)。

## 剩下的(需要 Ting 裁,已列 TING_PENDING_RULINGS C6/C7)

卡面還空著的幾格 —— 藥用部位 232、炮製英文 349、體質類型 211、方劑加減英文 214、西藥黑框查證註記 58 ——
**都需要新來源,不是搬運**。我不會自己編。要開哪一條由她決定。
...
`herb_pairs.teaching_note_zh` 會渲染成卡上的「學習提示」,但它長年被當成工程筆記在用:

- 「found_in_formulas 留空,不填未建之方」
- 「herbs 陣列用 herb.lu_gen,不另建 herb.wei_jing」
- 「故本記錄不標 board_exam_pair／ncbahm_official_pair」

讀卡的人不知道 `found_in_formulas` 是什麼,也永遠看不到 `herb.wei_jing` 這個 id。

更糟的是**這種句子會腐爛而沒人發現**。四條寫著
「宣稱本身留在藥卡標籤上未動,待 Ting 裁定該標籤怎麼改」——
那些標籤早在別的批次被遷走了(香薷/厚朴/白扁豆三張卡的 `key_pairs` 現在都是 0 條,
細辛卡的標籤今天剛換成「細辛 + 乾薑 + 五味子(小青龍湯)」)。
卡上因此長期掛著一句已經不成立的話,而且是「待某人裁定」這種會讓人真的去等的話。

## 怎麼改的:先搬再改,一個字不丟

原文**逐字**封存到新欄位 `curation_note_zh`(不渲染),
`teaching_note_zh` 換成讀者版。斷言擋著「封存必須與原文逐字相同」。

- 第一批 28 條:逐條手寫,因為每條的實質內容都不同。
- 第二批 13 條:兩種固定樣式,方名從原文抽,不手寫。
- 其中 1 條(茯神配酸棗仁)整段都是內部作業記錄,**移除欄位** ——
  渲染器只在有值時才畫那一區,所以卡上不再有「學習提示」,原文仍在封存欄。

**留下來的是讀者要的,不是我要的。**例如車前子配木通那條,
「木通類品種差異涉及馬兜鈴酸安全議題」必須留(那是安全訊息),
但「本庫八正散組成用的即為 herb.mu_tong」改寫成「八正散原方用的也是這一味」。

## 我的探針漏報了 13 條,閘門比我嚴

第一批做完我跑自己的探針,得到「帶工程痕跡 0 條」。
**那是錯的** —— 我的標記表沒收 `board_exam_pair`、`ncbahm_official_pair`、`schema_note`
這三個欄位名。新寫的閘門一跑就抓到 13 條。同一天第三次:
量測工具比資料先壞(參見 08-28 覆蓋率報表那次)。

第二批處理時又踩一次:模板外的尾巴有**兩種**
(「方中作「甘草梢」」= 炮製部位;「方劑卡未載角色說明的藥味:荊芥」= 這段只涵蓋兩味中的一味),
我第一版只抓前者,三條的後者會被留在幕後。兩種都抓才補上。

## 新閘門:validate-card-text-audience.js

只守一件事:`herb_pairs.teaching_note_zh` 裡不准出現**欄位名與記錄 id**。
上限 0。刻意**不**收「Appendix B」「NCBAHM」這類考綱專有名詞 ——
那是讀卡的人該看到的字,不是工程術語。

負向測試三種壞法(塞回藥卡 id、欄位名、藥對 id)都會 FAIL 並指名,乾淨時 PASS。
已註冊進 CLAUDE.md。

**這是窄閘門**,只管 herb_pairs 這一個欄位。別的資料集有沒有同樣的問題沒有量,
不在這裡假裝已經守住。

14 支驗證器 exit 0,棘輪 PASS。車前子卡開起來讀過:
兩條學習提示都是人話,馬兜鈴酸那句安全訊息留著,全卡沒有任何欄位名或 id。
...
**這 86 筆裡有 37 筆從頭到尾就沒錯,是我用同一把尺量三種結構不同的文件量出來的。**

## 三種來源要三把尺

| 來源型態 | 判準 | 為什麼不能共用 |
|---|---|---|
| 經絡課件(一穴一列的表) | 該穴自身那一列在哪頁 | — |
| 合訂本 `AP Point Book` | 專論頁 = 行首是代號 **且**其後(濾掉空行)3 行內有 `LOCATION:` | 只看「任一行行首是代號」會撞到索引頁與別的穴專論裡的交叉提及;只看「該頁第一行」又太嚴(一頁可排兩則專論) |
| 散文講義 `Therapeutics Notes` / `Techniques 3 points` | 被引的那一頁有沒有提到這個穴 | 散文沒有「條目」這回事 |

另外 `compare_with` 欄位還要第四把尺:它引的是**對比對象**所在的頁,不是自己
(ST36 配 SP6 就引脾經 SP-6 那一頁)。

分完之後:散文 11 筆**全對**(原本被我報成 8 筆缺陷);合訂本從「43 筆找不到」
變成 18 對 / 25 錯。
...
| 類別 | 筆數 | 例 |
|---|---|---|
| 經絡課件頁碼錯 | 23 | 膽經 GB26–GB44 全體 +1(寫 p.4 → 實際 p.5);腎經 KI22–KI27 寫 p.5/6 → 實際 p.4 |
| 合訂本專論頁錯 | 25 | BL62 p.198→p.196、CV12 p.70→p.330、HT7 p.239→p.120、LI4/5/11/17/18 全寫 p.32(章首頁)→ 各自專論頁 |
| `compare_with` 引錯頁 | 1 | ST36 配 SP6:寫脾經 p.2 → SP-6 在 p.1 |
| `compare_with` **引錯檔** | 1 | BL40 配 BL60:原引**肺經** p.1,但 UB-60 與註記引文「pain anywhere along spine」都在**膀胱經 p.8**(肺經全檔 0 次)→ 連檔一起換 |

改 119 處,`git diff` 加 119 / 減 119 相等 = 純取代無增刪行。

## 我的掃描器這一輪又錯了三次(累計六次)

同一種病:**用 regex 猜代號寫法,每次都漏一種**。

1. 只認 `LR` → 257 筆誤判(肝經課件寫 `LV-13`)
2. 補了六組前綴,還是漏 → 分隔符也會變(**同一份膽經課件**前段 `GB-26`、後段 `GB 26*`)
3. 放寬分隔符,還是漏 → `AP Point Book` 寫 `U.B. 62`、`L.I. 11`、`S.I.`、`St`、`Sp`、`Lu`、`Ht`、`P`、`Liv`、`Ren`、`K`、`SJ`

最後改成**兩邊都正規化**(去點/空格/連字號後比對)+ 別名表,才穩定下來。
教訓:**漏認比報不出來更貴** —— 它會把好資料報成缺陷,然後你去「修」沒壞的東西。

## 最後 6 筆是工具判不了的,用手驗

`compare_with` 那 6 筆我的稽核工具給不出乾淨答案(第四把尺的補丁反而把散文那邊
弄壞了),所以不再修工具,直接用**逐頁列出全部行首代號**這種沒有 regex 疑慮的方法
人工確認,6 筆全部成立:

| 卡 | 對比對象 | 引用 | 對象實際位置 |
|---|---|---|---|
| BL40 | BL60 | 膀胱經 p.8 | `UB-60*` 在 p.8 ✓ |
| LU6 | LI7 | 大腸經 p.2 | p.2 = LI-5…LI-10 ✓ |
| LU11 | LI1 | 大腸經 p.1 | p.1 = LI-1…LI-4 ✓ |
| ST25 | SP15 | 脾經 p.2 | p.2 = SP-8…SP-16 ✓ |
| ST36 | SP6 | 脾經 p.1 | p.1 = SP-1…SP-7 ✓ |
| LI18 | ST9 | 胃經 p.2 | p.2 = ST-9…ST-18 ✓ |

## 驗證

`build-data.js` + 8 支 validator 全 exit=0;棘輪 13 條全 flat
(`curriculum_anchors` 維持 0)。

**沒有把這四把尺做成 repo 裡的驗證器**:那需要把「哪個檔算哪一型」的分類與
`compare_with` 的語意都寫成規則,而我這一輪的經驗是這種分類很容易漏一種寫法。
現有的 `validate-curriculum-anchor-resolution` 仍只保證「位置存不存在」,
不保證「指對」—— 這一點沒有變。

# 2026-09-01 — 最後 34 條「考綱列名對藥」撤標;上限降到 0,考綱歸屬線收乾淨

Ting 裁定把上一則量到的 34 條也處理掉。這批比前兩批單純,因為量出來三件事:

1. **34 條的 `ncbahm_official_pair` 本來就都是 `false`** —— 唯一的宣稱是
   `relation: pair.rel.board_exam`,也就是卡上那個「考綱列名對藥」標籤。
2. **來源全部是 Bastyr**,不是 NCBAHM。
3. **`sources` 裡本來就誠實寫著**「Bastyr 考綱正本不在 curriculum/board/,本條未能對正本核讀」。

換句話說:資料層一直是誠實的,**只有畫面在說謊** —— 一個從來沒人核對過的
relation 值把「未能核讀」渲染成「考綱列名對藥」。

## 分三桶處理,一條都沒刪

| 桶 | 條數 | 處置 |
|---|---|---|
| 內含 Appendix B 二味對藥 | 10 | 指向那筆記錄,卡上寫明「考綱要背的是那兩味」 |
| 有策展過的 `found_in_formulas` | 11 | 卡上寫明是哪一首方的組成 |
| 兩者皆非 | 13 | 只寫「當常用臨床對藥記」 |

全部:移除 `relation`(卡上不再出現那個標籤)、
`official_claim_status` → `withdrawn__bastyr_source_not_in_repo`、註記附加。
原有註記整段保留(16 條有,逐條確認沒有一條因為這次改動而變假),
斷言擋著「註記只准變長」。內容欄位 17 欄逐一比對零改動。

**方劑那句只用策展過的 `found_in_formulas`,不用電腦算出來的「組成含這些藥的方」。**
掃描能算出 13 條有方劑可掛,但其中 2 條的 `found_in_formulas` 是前人**刻意留空**的
(「代表方菖蒲鬱金湯本庫尚無方劑卡,不填未建之方」)。而且大方子含當歸、川芎
不代表它是代表方 —— 那樣寫會把弱關係講成強關係。所以少報 2 條,不多報。

## 順手把自己上一批的形狀債還掉

上一批新增的 `contains_ncbahm_official_pair` 是**單值字串**。這批有一條
(延胡索配當歸、川芎、香附)同時內含「當歸+川芎」與「當歸+香附」兩組考綱對藥,
單值裝不下。硬塞就是這個庫最貴的那個病:同一欄位兩種形狀。
所以整批遷成陣列 `contains_ncbahm_official_pairs`,上一批那 9 條一併遷移,
驗證器把舊的單值鍵列為**明確錯誤**(不是靜默相容),指標數 9 → 20。

## 閘門

`BOARD_REL_CEILING` 34 → **0**:`pair.rel.board_exam` 只能用在真的在 Appendix B 上的
組合,一條都不准例外。現況 18 條,18 條全部核得到。

負向測試四種壞法,全部會 FAIL 並指名:上限被突破(0→1)、還在用單值舊鍵、
指標是空陣列、指標指向不存在的記錄。乾淨時 PASS。

## 三批的總帳

| | 8-27 之後 | 現在 |
|---|---|---|
| `ncbahm_official_pair:true` 不成立 | 14 | **0** |
| 藥卡標籤宣稱 NCBAHM 不成立 | 4 | **0** |
| `relation` 印「考綱列名對藥」而核不到 | 39 | **0** |
| 需要「已標未確認」放行的 | 18 | **0** |

## 沒動、要記著的兩件

**`board_exam_pair: true` 還在這 34 條上(全庫 101 條)。**grep 全 repo 沒有任何消費者,
語意也不清楚(「考綱列名」還是「值得為考試背」)。沒有依據就不亂翻,留著另議。

**16 條的既有註記是寫給工程師看的**,而它們會上卡片,例如
「found_in_formulas 留空」「herbs 陣列用 herb.lu_gen,不另建 herb.wei_jing」。
那是前幾批留下的,不是這次造成的,本批只保留未改寫。要不要整理是另一條線。

13 支驗證器 exit 0,棘輪 PASS。訶子卡與延胡索卡都開起來讀過:
「考綱列名對藥」確實不再出現,子對指路那句正常顯示。
...
我 8-27 的理由是「可能出自 NCCAOM 或其他考綱版本,那些正本不在 repo」。
一看 `curriculum/board/`:裡面是四份 **NCBAHM**(CH / ACPL / FOM / BIO),
**沒有 NCCAOM 正本**,而且只有 CH 那份有對藥附錄。

更要緊的是那個資料夾自己的 README 早就寫了規則:

> 卡片曾經硬寫過「Bastyr Exam Pearl」「NCCAOM actions」這種標籤,但那些值其實不是
> 從考綱來的。⋯⋯只有在真的能指到這個資料夾裡某份文件時才填⋯⋯**指不到就留空**。

我 8-27 的「可能是 NCCAOM」正是這條規則點名的那種推測。

## 9 條全部對得上方,不必刪任何東西

| 組合 | 代表方 | 本庫方劑卡 |
|---|---|---|
| 丹參配檀香、砂仁 | 丹參飲 | `formula.dan_shen_yin`(3 味,完全吻合) |
| 五靈脂配蒲黃 | 失笑散 | `formula.shi_xiao_san` |
| 豬苓配茯苓、澤瀉 | 四苓散(無卡)→ 五苓散 | `formula.wu_ling_san` |
| 澤瀉配白朮 | 澤瀉湯(無卡)→ 五苓散 | `formula.wu_ling_san` |
| 茯神配酸棗仁、遠志 | 歸脾湯 | `formula.gui_pi_tang` |
| 羌活 + 獨活 | **羌活勝濕湯** | `formula.qiang_huo_sheng_shi_tang` |
| 細辛 + 乾薑 + 五味子 | 小青龍湯 | `formula.xiao_qing_long_tang` |
| 滑石 + 甘草 | 六一散 | `formula.liu_yi_san`(2 味,完全吻合) |
| 延胡索 + 川楝子 | 金鈴子散 | `formula.jin_ling_zi_san`(2 味,完全吻合) |

所以處置是**把核不到的考綱宣稱換成核得到的方劑歸屬**,不是刪。
5 條 herb_pairs:`ncbahm_official_pair` → false、`official_claim_status` →
`withdrawn__no_board_source_in_repo`、移除 `relation`、重寫註記。
4 條藥卡標籤:`pair` 字串把「(2026 NCBAHM Appendix B 官方對藥)」「(Bastyr / NCBAHM 官方對藥)」
換成方名,`rationale_zh` 改寫。內容欄位逐一比對零改動(herb_pairs 17 欄;藥卡除 key_pairs 外全欄)。

**順手抓到一個獨立錯誤**:羌活卡原寫「代表方:九味羌活湯」,但九味羌活湯組成是
羌活、防風、蒼朮、細辛、川芎、白芷、黃芩、生地黃、甘草 —— **沒有獨活**。
同時含羌活與獨活的是羌活勝濕湯,已更正。

## 真正的問題比那 9 條大 4 倍,而且閘門看不見

改完之後前三條檢查全部歸零,看起來很乾淨。但考綱宣稱**不只寫在那個看不到的布林上**,
也寫在 `relation` 裡:`pair.rel.board_exam` 在卡上印出來的是
「**考綱列名對藥 · Board-outline listed pair**」—— 那才是使用者真正讀到的那句話。

| | 條數 |
|---|---|
| `relation = pair.rel.board_exam` | 57 |
| 其中核得到(在 Appendix B 上) | 18 |
| **核不到** | **39** |
| 核不到而且完全沒有揭露標記 | 34 |

前三條檢查只查布林與標籤字串,所以這 39 條**一條都沒被看到**。
修完 Ting 指名的 5 條後還剩 34 條,卡上仍印著「考綱列名對藥」而無從佐證。

`validate-board-pair-attribution.js` 補第四段:照本 repo 既有做法收 ——
**具名上限 34、只准變少、超過就 FAIL**。負向測試:乾淨時 34 PASS,
故意把一條核不到的記錄改成 `board_exam` relation → 35,FAIL 並指名。
不列逐條清單是因為那 34 條要逐條查證來源(多半是課件或方劑組成),那是另一批工作;
先把「不准再新增」鎖住。**這 34 條要不要處理,等裁定。**

## 又一次:保留下來的舊句子變成假的

5 條裡有 3 條的註記前面有原始遷移說明,我保留了。但五靈脂那條寫著
「relation 仍用 `pair.rel.board_exam` 與同批一致」—— 我這次正好把 relation 移除,
那個子句當場變成假的。留一句不成立的話比刪掉更糟,所以只拿掉那個子句,
同句裡「相須為用」的實質內容保留。另外兩條的前綴仍為真,原樣留著。

## 另外量到、沒動的

223 首方劑裡有 **2 首名稱是簡體**:`formula.fang_feng_tong_sheng_san`「防风通圣散」、
`formula.qiang_huo_sheng_shi_tang`「羌活胜湿汤」。這是繁體庫裡的異物,但屬於命名線,未動。

13 支驗證器 exit 0,棘輪 PASS。滑石卡與五靈脂卡都開起來讀過:
新標籤正確,五靈脂那條的「考綱列名對藥」標籤確實不再顯示。

# 2026-09-01 — 171 筆「頁碼在範圍內卻指錯」的穴位錨點修完(684 處),指錯歸零

上一批查 7 個 A3 時順手量出的:`361.json` 的 `#p` 錨點裡有 171 筆頁碼在範圍內、
卻不是該穴自身條目所在的頁 —— 驗證器結構上看不到。Ting 裁定修掉。

## 逐穴定位,不是套位移

**偏移方向各經不同**(膀胱 +1/+2/−1、小腸 −1~−3、督脈 +1、胃 −1…),
不是單一公式算錯,所以不能用一個位移一次修完。每個穴各自去 PDF 裡找自身條目。

判準:**行首就是該穴代號**。課件代號與卡片 id 不同,對照表是實際從 PDF 撈出來的:
`BL→UB`、`KI→KD`、`CV→REN`、`GV→DU`、`TE→SJ`、`LR→LV`。

改了 **684 處 / 171 組(穴, 錨點)**(每個穴約在 4 個 `field_sources` 欄位引用同一錨點)。
逐經:膀胱 217、督脈 68、小腸 60、脾 49、胃 45、大腸 44、三焦 40、任 33、腎 28、
肝 24、肺 22、膽 21、心包 17、心 16。

## 唯一需要人裁定的一筆

BL34 的自身條目橫跨兩頁:p.5 那一行 `UB-34` 後面**沒有內容**(頁緣斷行殘留),
p.6 才是 `UB-34  Local Point / Xia4 Liao2  • On sacrum  • Regulates Lower Jiao`。
人工確認後寫死成 p.6,腳本裡明講「跨多頁又沒有人工裁定就 throw,不猜」。

## 驗證:不能只讓掃描器同意自己

重跑稽核:**339 正確、指錯 0**(86 筆「找不到」未動,見下)。
但那是同一支程式自我確認,所以另外**獨立手抽 7 筆**,直接抽該頁列出行首代號:

| 穴 | 改後 | 該頁實際 |
|---|---|---|
| BL9 | p.2 | `UB-9  Local Point` ✓ |
| BL34 | p.6 | `UB-34  Local Point` ✓ |
| SI5 | p.1 | `SI-5  • At ulnar end of transverse` ✓ |
| ST25 | p.3 | `ST-25**` ✓ |
| GV14 | p.3 | `DU-14**  • REN-17, ST-9, DU-15` ✓ |
| TE5 | p.1 | `SJ-5*  Luo Connecting Point` ✓ |
| CV12 | p.3 | `REN-12**` ✓ |

第一次手抽時有 3 筆回報「找不到」,查下去是**我的 shell 檢查寫錯**:條目寫成
`ST-25**` 兩個星號,而我的 `\*?` 只允許一個。逐頁 dump 全部代號才確認胃經 p.3
就是 ST-19~ST-28、督脈 p.3 就是 DU-10~DU-14。**核對工具自己也會錯**,
所以最後用的是「列出該頁全部行首代號」這種沒有 regex 疑慮的方法。

## 沒有動的 86 筆

`361.json` 裡還有 86 筆「該 PDF 找不到這個穴的條目」。要再拆:
43 筆在 `AP Point Book (1).pdf`(多經合訂本)、8 筆在 `Therapeutics Notes`(散文體,
穴不是以表格列出現)—— 這兩類多半是我的判準不適用,不是資料錯;
但也有少數看起來是真的引錯檔(`LU6`/`LU11` 指到大腸經課件、`BL40` 指到肺經課件、
`PC8` 指到心經課件)。判準要換,不在本批。

## 驗證

`build-data.js` + 10 支 validator 全 exit=0;棘輪 13 條全 flat
(`curriculum_anchors` 維持 0 —— 本批修的是它看不到的那一類)。
diff 684/684 相等 = 純字元取代,沒有增刪行。
開 app 看 BL9 穴卡:正常渲染、console 無錯誤、裸錨點沒有漏到畫面
(`field_sources` 不上畫面)。
...
`validate-curriculum-anchor-resolution` 從 7 降到 **0**,全部 371 個錨點都解析得到。

但同一套查法往外掃,量出一個驗證器**結構上看不到**的問題:171 筆錨點頁碼在範圍內、
卻指到錯的一頁。那批**沒有動**,見文末。

## 那 7 個錨點:逐穴定位,不是逐錨點

出問題的都是每條經最後幾個穴。同一個錯錨點底下的穴可能落在不同頁,所以逐穴改:

| 穴 | 原 | 改 | 課件 |
|---|---|---|---|
| LU5 | p.4 | **p.1** | 1 LUNG |
| LU10 / LU11 | p.4 | **p.2** | 1 LUNG |
| HT7 / HT8 | p.4 | **p.2** | 5 HEART |
| HT9 | p.5 | **p.2** | 5 HEART |
| PC7 / PC8 | p.4 | **p.2** | 9 PERICARDIUM |
| PC9 | p.5 | **p.2** | 9 PERICARDIUM |
| SP17–SP21 | p.5 | **p.3** | 4 SPLEEN |
| LR13 / LR14 | p.5 | **p.3** | 12 LIVER |

共 64 處、16 個穴。判準:**行首就是該穴代號 = 那是它自己的條目**。
這條規則有實例支撐 —— PC-7 在 p.1 以 `(PC-7)` 出現(別的穴的交叉引用)、
p.2 才是 `PC-7*  Shu Stream Point  • In middle of transverse wrist…` 它自己那一列。
LU5 也靠這條規則才對:寬鬆搜尋以為 p.2,嚴格看行首才發現自身條目在 p.1
(p.1 收 LU-1~LU-5、p.2 收 LU-6~LU-11)。

順帶去掉 2 個重複值(`LU5.compare_with`、`LU10.exam_pearl`):
`field_sources` 是併集語意,重複沒有意義。其中 LU5 那個是我改頁碼時造成的。

## 我的掃描器連錯兩次,都是同一種病

1. **代號寫法對不上**:肝經課件寫 `LV-13` 不是 `LR-13`,我搜不到就差點報成
   「課件裡沒有這個穴」。補齊對照表後才發現實際共有六組不一樣:
   `BL→UB`(檔名是 URINARY BLADDER)、`KI→KD`、`CV→REN`、`GV→DU`、`TE→SJ`、`LR→LV`。
   第一版只寫 `LR→LV`,**257 筆被誤判成「找不到」**,補齊後降到 86。
2. **偏移看起來很整齊時,先懷疑自己**:第一輪看到「一律差一頁」,那通常代表我對
   `#pN` 的定義理解錯了。所以先去查課件頁尾的印刷頁碼 —— p.1 頁尾就是「1」,
   與 PDF 頁序一致,`#pN` 的定義沒有偏移。確認之後那些差異才算數。

## 沒有動的:171 筆頁碼在範圍內卻指錯

把同一套查法套到 `361.json` 全部 438 個 `#p` 錨點:

| | 筆數 |
|---|---|
| 頁碼正確(該穴自身條目就在那一頁) | 181 |
| **頁碼指錯**(在範圍內,驗證器看不到) | **171** |
| 該 PDF 裡找不到該穴條目 | 86 |

手動核對過樣本確認不是誤判:UB-9 在 p.2(卡片寫 p.1)、UB-16 在 p.3(寫 p.2)、
SI-5 在 p.1(寫 p.2)、SI-9 在 p.2(寫 p.3)。

**偏移方向各經不同**(膀胱 +1/+2/−1、小腸 −1~−3、督脈 +1、胃經 −1…),
不是單一公式算錯,所以不能用一個位移一次修完 —— 要逐穴定位,就像這批的 16 個穴。
分佈:膀胱 55、督脈 17、小腸 15、脾 13、胃 11、大腸 10、三焦 10、任 8、腎 7、
肝 6、膽 5、肺 5、心 4、心包 4、Techniques 1。

那 86 筆「找不到」還要再分:43 筆在 `AP Point Book (1).pdf`(多經合訂本)、
8 筆在 `Therapeutics Notes`(散文體,穴不是以表格列出現)—— 這兩種多半是我的
判準不適用,不是資料錯;但也有少數看起來是真的引錯檔(`LU6`/`LU11` 指到大腸經課件、
`BL40` 指到肺經課件、`PC8` 指到心經課件)。要拆開才知道。

`data/acupoints/**` 是穴位線的檔,這批 64 處是依 Ting 裁定動的;
171 + 86 那批**沒有動**,留裁定。執行成本:機械可定位,方法與本批相同。

## 驗證

`build-data.js` + 驗證器全 exit=0;棘輪 13 條全 flat,`curriculum_anchors` **7 → 0** 已鎖。
diff 65/67(去重讓兩行變一行),非整檔重排。

**這也再次說明那支驗證器的定位**:它只查「位置存不存在」。天花板降到 0 的同一天,
同一個檔裡有 171 筆指錯的錨點它一個都看不到 —— 這句話本來就寫在它的檔頭與輸出裡,
現在有了具體數字。

# 2026-09-01 — 棘輪的「讀不到缺陷數」以前印一片空白;現在分三種故障各自出聲

接上一則留的未結項。那次 `validate-curriculum-anchor-resolution.js` 間歇硬當機,
棘輪只印了一句 `RATCHET ERROR: could not read a defect count`,**後面是空的** ——
我因此只能用猜的,還先猜錯了兩次(先怪自己的改動、再怪 `Math.max(...pages)`)。

根因在 `run()`:它用 `execFileSync` 包 try/catch,回傳 `stdout + stderr`,
**把離開碼整個丟掉**。硬當機時 stdout 與 stderr 都是空的,於是 `out.slice(0, 400)`
印出空白。改用 `spawnSync`,把 `status` / `signal` / `error` 一起帶回來。

現在分三種說:

| 情況 | 訊息 |
|---|---|
| 子行程起不來 | 印 spawn 的錯誤訊息 |
| 驗證器自己掛了 | 「**不是缺陷數變多,不要照著去改資料**」+ exit/signal/輸出長度 |
| 正常跑完但抽不出數字 | 「多半是報表格式改了而 `extract()` 沒跟上,不是資料變壞」 |

**負向測試當場打臉了我的第一版。**我原本用「輸出是不是空的」當判準,
三支假驗證器一跑,三條分支全落到同一句話:`process.abort()` 會吐 1385 字元的
原生堆疊,模組找不到會吐 823 字元的 loader 錯誤 —— 兩者輸出都非空,
只有真正的 `0xC0000409` 才是一個字都不印。

判準改成**離開碼**:正常跑完的驗證器只會是 exit 0(PASS)或 1(有缺陷),
其餘一律當它自己掛了;另加一條 node 崩潰特徵(`node:internal` / `Cannot find module`
/ `FATAL ERROR` / `Native stack trace`),因為「找不到模組」的離開碼剛好是 1,
會跟「有缺陷」撞號。三支假驗證器重跑,三條分支各說各的話。

那個間歇當機的**根因仍然未知**,這次只是把下一次的證據留住。

# 2026-09-01 — Task 11I 驗收落地:411 條董氏「非圖片」死連結,407 條找到同站真候選

antigravity 自發延續 Task 11G/11H 的模式(未經正式派工,巡檢旗標先記了一筆),交出
`data/audits/tung_dead_link_reference_candidates_2026-09-01.json`,對 11G 帳本裡
411 條非圖片(參考頁)死連結逐條找同站候選:**407 條找到真候選(全部驗證回 200)、
8 條查無**(6 條是原始網址本身格式壞掉——結尾多一個冒號或指到目錄頁而非穴位頁,
沒有對應的穴位頁可找)。

**獨立複核**(用真實瀏覽器而非 curl——這個 host 對非瀏覽器請求一律回 403,curl 直接測
會誤判成全部失效,必須用真的瀏覽器連線才看得到真正的回應):
- 抽查 3 條候選(`BL1`→`points/bl1`、`ex.b10`巨闕俞→去掉原網址結尾多餘冒號後的
  `points/juqueshu-ex-b14`、`TE23`→`points/th23`),**逐一開啟核對頁面標題與內容,
  三條全部吻合**(TE→TH 的命名差異在報告裡有解釋:該站三焦經用 th 不用 te)。
- `--self-test` 13/13、`check-validation-ratchet.js` 無退化。
- 407 個候選的 `card_id` 逐一核對存在於 `data/acupoints/**`,零缺漏。
- `git diff --stat origin/main..HEAD` 只有兩個新稽核檔(候選帳本 + 報告),
  `data/acupoints/**`、`app.js`、`js/**` 零改動——照邊界規矩只出候選,不改資料。

要不要真的把候選網址換上卡片,是後面的裁定,不是這輪的範圍。

MEASURED TREE: `origin/main` @ 68d7a5ed(落地前 base)
...
於是 `Diagnosis → Strategy → Formula` 在課件裡長成
`Diagnosis (cid:0) Strategy (cid:0) Formula`。

## 為什麼確定是 `→`(不是靠語感)

1. 全庫 1164 處**全部是同一個** `(cid:0)`,不是一堆不同字形壞掉。腳本先斷言這件事,
   若出現第二種 cid 就整支停 —— 那代表不只一種字形壞了,不能一律換成箭頭。
2. **同檔直接對照**:`10_Formula_Cards_091-100` L2191 是
   `Dao Chi San → Sheng Di, Mu Tong, Dan Zhu Ye, Gan cao???`,
   L2419 同一句是 `Dao Chi San (cid:0) Sheng Di, …`。
3. **系統性比對**:1003 行含 `(cid:0)`,把它換成 `→` 之後,
   **231 行在別處找得到逐字相同的行,零反例**。
4. `curriculum/formulas/` 用過的箭頭**只有 U+2192 一種**(原有 1821 處),沒有歧義。
...
| `12_Formula_Cards_111-120_理氣劑_瀉下劑.md` | 97 |
| `13_Formula_Cards_121-130_瀉下劑_和解劑.md` | 95 |
| `08_Formula_Cards_071-080_解表劑.md` | 77 |
| `07_Formula_Cards_061-070_祛濕劑_解表劑.md` | 51 |
| `04` 38 · `09` 35 · `03` 32 · `15` 25 · `17` 21 · `18` 21 | |
| `10` 18 · `14` 18 · `06` 15 · `20` 15 · `02` 14 · `16` 9 · `19` 7 · `05` 6 · `11` 6 | |
| **合計** | **1164 處 / 21 檔** |

## 三道斷言(任何一條不過就整支停)

- **行數不能變**:`data/**` 有 `#L` 錨點指向這些檔,行數一動就全錯。逐檔比對行數。
- **除了這個取代不准有別的差異**:把 `(cid:0)` 與 `→` 都剝掉後前後文須逐字相同。
- **長度差要剛好等於 次數 ×(7−1)**。

收尾核對:剩餘 `(cid:N)` **0** 處;`formulas/` 箭頭總數 **1821 + 1164 = 2985**,對上。
`git diff --numstat` 新增 1003 行、刪除 1003 行 —— 相等,證明是純字元取代沒有增刪行。

## 刻意沒有加出處註記

跟前一批的 CJK 回填不同做法,理由:插入漢字是「可能被懷疑手打」的,需要註記背書;
把壞掉的字形修回箭頭是自證的(`Too deficient to be tonified → Use Sha Ren`),
再加 21 段中文註記反而會污染「全庫 CJK 總數」這個我們一直在用的指標。出處記在這裡。

## 順手補上閘門的一個缺口

`curriculum/formulas/` 磁碟上只有 .md,沒有二進位來源(來源只在 git 歷史),
所以 `extract-curriculum-md.js` 現在掃不到、覆蓋不了。但寫這則紀錄時發現:
**如果哪天有人把來源從 git 取回再跑 `--force`,防退化閘門擋不住這一項** ——
閘門看中文數、壞字元數、去空白內容量,而重抽回 `(cid:0)` 在這三項上都不算變差
(`(cid:0)` 七個字元,比 `→` 一個字元**還長**,內容量反而增加)。
**壞掉的輸出不一定比較短**,這是閘門第一版沒想到的。

所以在 `qualityOf()` 加了第四項 `cid`,`writeIfNotWorse()` 加一條
「未對映字形變多就拒寫」。**實測過會開火**:把已修好的檔餵回 `(cid:0)` 版,
回傳 `ok=false / 未對映字形 (cid:N) 0 → 2`,檔案內容維持箭頭沒被覆蓋。

(差一點就漏了:第一次改的時候只加了檢查那一行、忘了加指標本身,
於是 `b.cid > a.cid` 兩邊都是 `undefined`,比較永遠 false —— 檢查等於不存在,
而且不會報錯。是實測才抓到的,不是看程式碼看出來的。)

## 驗證

`build-data.js` + 驗證器全 exit=0;棘輪 13 條全 flat;
`validate-curriculum-anchor-resolution` 仍是 7(A3),行數沒變所以錨點不受影響。

# 2026-09-01 — 蘇子/紫蘇子重複卡照 D21 合併:16 欄遷入,劑量兩欄刻意不遷

Ting 裁定「照 D21 合併」。`herb.su_zi`(蘇子)與 `herb.zi_su_zi`(紫蘇子)是同一味的
重複匯入記錄:兩張的 `cautions`／`clinical_use_note`／`chinese_depth_track` 通篇寫的都是
「紫蘇子」,分類同為化痰止咳平喘藥,來源同為 CloudTCM `herb/1254`。

**保留 `herb.zi_su_zi`**,兩個理由:它是藥典正名(Perillae Fructus),而且它是**被引用的
那一張**(藥對 2、方劑 3:蘇子降氣湯／三子養親湯／定喘湯);`herb.su_zi` 零引用。
改 id 會斷外鍵(紅線一),所以合併方向由引用決定,不由名字好聽決定。

## 逐欄數字

| | 合併前 | 合併後 |
|---|---|---|
| `herb.zi_su_zi` 有值欄位 | 33 | 51 |
| `herb.zi_su_zi` `cautions_zh`／`_en` | 1／1 | 6／6(等長) |
| `herb.zi_su_zi` `field_sources` | 2 | 6 |
| `herb.su_zi` `review_status` | `draft` | `deprecated` |
| `herb.su_zi` `deprecated_note_zh` | 0 字 | 1777 字 |
| 全庫筆數 | 366 | 366(未硬刪) |

遷入的 16 欄在該卡原本**全為空**,不是覆蓋。多出來的安全內容是紫蘇子卡本來沒有的:
孕婦慎用(辛溫活血)、藥物交互作用、過量致瀉、**生紫蘇子有毒須炒製或煎煮**;
`safety_flags` 的 `pregnancy_review` 一併帶入。`cautions` 採**併集**(該卡原有 1 條在前),
原值依 archive-before-replace 封存於該卡自身 `import_artifacts`。

自我 diff:動 2 張(全庫 366 張逐筆比對),**變短的欄位 0 個、被清空的欄位 0 個**,
遷入全部是 0 → N 的純新增。

## 兩件刻意不遷的

**`related_formulas`**:`herb.su_zi` 列 7 首,逐一核對 `composition` 後只有 3 首確實含
紫蘇子(蘇子降氣湯、三子養親湯、定喘湯 —— 該卡本來就有這 3 首)。二陳湯、溫膽湯、
貝母瓜蔞散、杏蘇散 4 首的組成皆不含紫蘇子(杏蘇散用的是蘇葉不是蘇子),判定為未查證
關聯,未遷移亦未修改,比照 D21 對沙參那次的處置。

**`dosage`／`dosage_normalized`** —— 這條是這批唯一被閘門擋下來的,值得記:
第一版原樣搬了,`validate-herb-dosage-shape` 立刻 FAIL:「JSON 字串」形狀從 176 增加到 177。
查下去才看清楚形狀分布是這樣的 ——

- 177 張「JSON 字串」解析後**全是同一組鍵**:`一般建議｜特殊說明｜食療用量範圍`
- 116 張「物件」用的是**完全不同**的鍵(`decoction_g｜特殊說明`、`max_g｜min_g｜route｜unit`…),
  沒有一張是上面那組鍵

所以「往物件收斂」對這一張的意思,是在 177 張的世代裡生出**獨一無二的第三種形狀** ——
正是本庫反覆付代價的那個缺陷。加上這 177 張同屬一個待裁世代(D3 食療上限高於藥用上限
75 張、蘇合香 SAFETY_HOLD 未決),該由該世代自己的批次一起收斂,不該由這次合併單方面
動安全欄位(紅線四)。故兩欄都不遷。

**代價要講清楚,這不是零損失**:`herb.zi_su_zi` 的 `dosage` 原本就是 `null`,不遷之後
它仍然是 `null`,紫蘇子卡沒有劑量值。今天沒有畫面後果 —— 卡上「常用劑量」區塊讀的
不是 `record.dosage`,實測有 `dosage` 的蘇子卡同樣顯示「生藥日服量:待補」。
原值逐字留在退役卡上,**劑量世代裁定後應一併回填 `herb.zi_su_zi`**,已寫進退役註記。

## 驗證

13 支驗證器全部 exit 0;棘輪 PASS,`herb_canon` 缺陷 **5533 → 5495(−38)**。
用眼睛讀了兩張卡:紫蘇子 6 條慎用雙語齊全、無樣板句、無生 enum;蘇子的狀態丸正確
印出「已退役 Deprecated」。

## 一個沒查出根因的東西

`validate-curriculum-anchor-resolution.js` 曾硬當機一次(exit `-1073740791`),當下我判定
是自己的改動造成的,**這個判定是錯的**:那支跑 3 次是 0 / 當機 / 0,間歇性。
之後連續乾淨跑 27 次(`--json` 12、報表 12、加上前面 3 次裡的 2 次),沒能重現。
分階段複現也排除了兩個嫌疑:整檔正則與 JSON 遞迴走訪各跑 5 次全乾淨;
`Math.max(...pages)` 展開的是最多 41 個元素的 Set,不可能爆堆疊(我先前指認它是錯的)。
`0xC0000409` 在 Windows 上是 node 的通用 abort 碼,不專指堆疊溢位。
**根因未知,留著。**這支是棘輪讀數字的來源,它一當機棘輪就報「讀不到缺陷數」——
也就是說它會發假紅燈,而本專案對閘門的要求是不准發假紅燈。下次再遇到要抓 stderr。
...
## 刪除前最後一次用組成核對(三條理由逐一成立)
| 條目 | 敘述宣稱 | 該方實際組成 |
|---|---|---|
| 天王補心丹／柏子仁 | 「方中含有**黃連、黃芩**等清熱燥濕之品」 | 13 味:酸棗仁、五味子、柏子仁、丹參、茯苓、桔梗、麥門冬、人參、生地黃、天門冬、玄參、遠志、當歸 —— **無黃連、無黃芩** |
| 天王補心丹／五味子 | 「可與**方中**其他補腎藥物如**黃芪**、茯苓等相輔相成」 | 茯苓在、**黃芪不在** |
| 定喘湯／款冬花 | 「與**黃芪**、甘草等藥物配伍」 | 9 味有甘草、**無黃芪** |
三筆都是「把不存在於本方的藥味說成本方組成」,而且整段推論建立在那個不存在的組成上。

## 作法與上一批一致:先封存再清空
原文(242 + 269 + 171 = 682 字)逐字封存進各自方劑的 `correction_note`,再刪欄位。
**動到 2 首方,變動欄位只有 `composition` 與 `correction_note`**,其餘 221 首逐筆零改動;
被改的那幾味除了少掉 `elucidation_zh`,其他鍵一字未動(寫入前 assert)。

**中途斷言擋下我自己一次**:第一版對**每一味**都剝掉 `elucidation_zh` 再比對鍵集合,
於是沒被我改、但本來就有那個欄位的味(天王補心丹的酸棗仁)被誤判成「鍵有異動」而中止。
資料沒被寫入。修的是斷言不是放寬它 —— 改成只對指定的那幾味放行該鍵。

**眼讀**:天王補心丹卡內已無「黃連、黃芩等清熱燥濕」與「補腎藥物如黃芪」;
定喘湯卡內已無「與黃芪、甘草等藥物配伍」。兩張卡照常渲染。

## 掃描結果:35 → 31 筆,剩下 1 筆命中的不是假警報
重掃後只剩「定喘湯／紫蘇子」被標(內文寫「蘇子」)。我原本判它是假警報——
**蘇子是紫蘇子的簡稱,只是別名沒登記**。查下去發現不是:

**庫裡有兩張卡。** 這是第 5 組重複卡(前 4 組烏賊骨/海螵蛸、茜草根/茜草、旱蓮草/墨旱蓮、
沙參/北沙參已於 D21 處理):
| | 欄位/有值 | source_type | 被引用 |
|---|---|---|---|
| `herb.su_zi` 蘇子 | 51 / **49** | sourced_cloudtcm_record | **0**(藥對 0、方劑 0) |
| `herb.zi_su_zi` 紫蘇子 | 35 / 33 | formula_ingredient_gap_fill | 藥對 2、方劑 3(蘇子降氣湯、三子養親湯、定喘湯) |
兩張都不是 `deprecated`,都沒有 `aliases_zh`。**形狀跟旱蓮草那組一模一樣:
內容較完整的那張反而零引用**(D21 那次的處置是把完整的內容整批遷入被引用的那張,再把前者退役)。
**合併是刪除,依憲法要你裁定,本批一個字都沒動**,也沒有貿然加別名 ——
在兩張卡並存的情況下加「蘇子」別名會製造歧義。
...
`git diff --check` 無輸出。本批只動 `data/herbs/formulas.json` + generated 隨批重建。

## 待裁
`herb.su_zi` / `herb.zi_su_zi` 要不要照 D21 的作法合併(完整內容遷入被引用的那張,另一張退役)?
...
## 一、那 9 條:確認是真前向引用,無雙胞胎可救
7 首方逐一查過(id 比對 + 名稱比對 + 別名比對):人參蛤蚧散、交泰丸、苓甘五味薑辛湯、
桂枝加龍骨牡蠣湯、橘核丸、麻黃連翹赤小豆湯、葶藶大棗瀉肺湯 —— **223 首方裡一首都沒有,
也沒有任何同名/近似的替代 id**。所以不是重導得掉的東西,是真的沒建卡。
前批已裁定「留前向引用不補骨架」,維持;差別是現在有 gate 盯著,只准變少。

## 二、真正的發現:這個欄位沒有任何地方讀它
`herb_pairs.json` 的 schema_note 明寫用途是
「formula ids where this pair does the work, **so the formula card can show its pairs**」。
全庫 grep(`js/**`、`app.js`、`scripts/**`):**除了我這支新驗證器,零消費者**。
| | 條數 | 涵蓋方劑卡 | 有沒有渲染 |
|---|---|---|---|
| 方劑側 `key_pairs` | 25 | 9 張 | **有** |
| 藥對側 `found_in_formulas` | 274 | **123 張** | **沒有** |
策展量差 11 倍,而且**多的那一側是 schema 指定要餵方劑卡的那一側**。
如果接上去(與 key_pairs 併集):**120 張方劑卡會多出 240 條藥對,其中 114 張目前
`key_pairs` 完全是空的** —— 那 114 張現在只能印「依組成推得」的候選或空白。
名單裡是防風通聖散、半夏白朮天麻湯、十全大補湯、五苓散、普濟消毒飲、葛根湯、歸脾湯、
補中益氣湯、溫經湯、八珍湯、血府逐瘀湯、逍遙散這一類考科主力。

## 三、但**不能直接接**:先量了可信度,還不到可以接的程度
逐條驗「藥對的每一味是不是真的在該方 composition 裡」,265 條可查證連結:
**244 條成立、21 條不成立**。21 條分三類,性質差很多:
| 類 | 條數 | 性質 |
|---|---|---|
| 兩味都不在該方 | **5** | 幾乎確定是錯連:麻黃配石膏→越鞠丸、人參配黃芪→半夏白朮天麻湯、巴戟天配肉蓯蓉→右歸丸、生薑配大棗→參苓白朮散、三棱配莪朮→至寶丹 |
| 單味不在該方 | **16** | 混合:部位別(栝樓皮→小陷胸湯用瓜蔞、槐米→槐花散用槐花)、方劑組成可能漏味(白果→定喘湯)、疑似錯連(附子配乾薑→理中丸缺附子) |
接上去 = 把沒驗過的資料當成已驗過的送上畫面。**那是 CLAUDE.md 第 5 條的反面** ——
不是 fallback 說謊,是把未驗內容當已驗送出去。所以先量起來、盯住,接線與否留 Ting 裁。

**中途訂正自己的量測**:初版比對抓到 36 條不成立,其中 **15 條全部是「甘草 vs 炙甘草」** ——
那是**炮製寫法差異不是內容錯**,連結在臨床上成立(桂枝湯的芍藥甘草本來就用炙甘草)。
加上炮製前綴比對後 36 → 21。**但部位別不併**(栝樓皮 ≠ 瓜蔞、槐米 ≠ 槐花):
那是不同藥材,判成相同會把真的錯連洗白。

## 四、新 gate:`scripts/validate-found-in-formulas-integrity.js`
三個計數各一個上限、**只准變少**:`dangling 9` / `mismatch 16` / `both_missing 5`。
比對吃「同一味藥兩張卡」與「炮製前綴」,不吃部位別。讀不到 pairs/formulas/herbs 一律 FAIL(不准空跑)。
檔頭把「這個欄位目前是暗的」與「為什麼還不能接」寫死在註解裡,免得下一個人看到 274 條就直接接上去。
**負向測試**:注入「麻黃配桂枝 → 四君子湯」→ `FAIL — both_missing 6 > 5`;還原 → PASS,資料零殘留。

## 五、驗證(九支全跑)
`validate-herb-standard` / `validate-formula-standard` / `validate-content-junk` /
`validate-herb-pair-render` / `validate-board-pair-attribution` /
`validate-review-status-vocabulary` / `validate-rendered-reference-resolution` /
`validate-found-in-formulas-integrity`(新) / `check-validation-ratchet` **全 PASS**;
`git diff --check` 無輸出。gate 已登記進 `CLAUDE.md`(70 行)。
**`data/**`、`js/**` 一個字都沒改。**

## 待 Ting 裁(三項,依價值排序)
1. **接不接 `found_in_formulas` 到方劑卡?** 接了 120 張卡多 240 條藥對(114 張從空的變有),
   但要先處理下面第 2、3 項,否則會把錯連送上畫面。
2. **5 條兩味都不在該方的錯連**:刪掉還是改指正確的方?(刪除需你點頭)
3. **16 條單味不符**:部位別要不要當成同一味(栝樓皮/瓜蔞、槐米/槐花)?
   白果不在定喘湯組成裡看起來是**方劑卡漏味**,那要修的是方劑側不是藥對側。

MEASURED TREE: claude/practical-easley-73f009 @ origin/main a67011aa + 本批
...
重映射沒做** —— 查根因時發現重映射會把工作做進一個劣化的檔案裡,先呈報。

## 驗證器 `scripts/validate-curriculum-anchor-resolution.js`

只問一件事:**錨點指的位置存不存在**。已接進 `check-validation-ratchet.js`
(`curriculum_anchors`,天花板 26)與 CLAUDE.md 驗證器清單;CI 走棘輪那一關,
數量不准變多。

它**查得到**:檔名改名/搬家(A1)、行號超界(A2)、頁碼超界(A3)。
它**查不到**:錨點指到的內容支不支持該主張。這點寫進檔頭與畫面輸出,
免得天花板降到 0 被誤讀成「錨點全對」。

兩個踩過的坑寫進實作:
- **頁標記有兩種格式**。初版只認 `## p.N`,`Therapeutics Notes` 用
  `--- Page N ---`,抽到 0 個頁標記就把它引用的 4 個錨點全判成壞的。現在兩種都認,
  那 4 筆假訊號消失(該檔實際有 142 頁,`#p101` 本來就成立)。
- **防空跑**:抽到 0 個錨點一律 `exit 2` 並明說「是解析器壞了,不是資料乾淨」。
  已實測(把 SCAN_DIR 指到 `.github`,exit=2)。

## 這一批修掉的:A1 檔案不存在 6 筆 → 0

| 原錨點 | 改成 | 依據 |
|---|---|---|
| `curriculum/acupoints/10 TRIPLE BURNER…SHAO YANG.pdf#p1‑p5`（92 處） | `10 SAN JIAO…SHAO YANG.pdf` | 同一個 merge(`e4006e8c`)裡刪舊增新 = 改名;新檔正好 5 頁,容得下 p1–p5 |
| `curriculum/Formulations Summary Chart.docx.md#L2188-L2206`（4 處） | `curriculum/formulas/…` | 少一層目錄;L2188–L2206 實際讀過,右欄即 Er Xian Tang 組成與出處(Fang Ji Xue),與 `formula.er_xian_tang.field_sources.source_classic` 相符 |

只改路徑、不動行號頁碼,腳本內斷言「長度變化 = 出現次數 × 路徑長度差」,
且目標檔必須真的存在,否則 throw(免得把壞引用換成另一個壞引用)。

棘輪 `curriculum_anchors` **32 → 26**(A2 19 + A3 7),已 `--update` 鎖定。

## A3 那 7 筆是真的,不是我誤判

經絡課件的 `#p4`/`#p5` 指到 `.md` 沒有的頁。我先懷疑是 PDF 有而文字層沒抽到,
於是直接讀 PDF 的 `/Count`:

| 檔 | PDF 實際頁數 | .md 頁標記 | 被引到 |
|---|---|---|---|
| 1 LUNG / 5 HEART / 9 PERICARDIUM | 3 / 3 / 3 | 3 / 3 / 3 | p4、p5 |
| 12 LIVER / 4 SPLEEN | 4 / 4 | 4 / 4 | p5 |

PDF 本身就沒有那幾頁,所以錨點是錯的。全部出自 `data/acupoints/361.json`,
該指到哪一頁要穴位線判斷,不是機械修得掉的,留在天花板裡。

## 根因:`ebef2401` 那次重抽是**倒退**,不只是行號位移

A2 的 19 筆全部指向兩個 Chenoweth 課件。比對舊版(`47833857`)與現行:

| 檔 | 版本 | 行數 | CJK 中文字 | U+FFFD 壞字元 | 空括號 `()` |
|---|---|---|---|---|---|
| `materia_medica_abbreviated_chenoweth.md` | 舊 | 9007 | **1110** | 0 | 0 |
| 同上 | **現行** | 3475 | **0** | **3582** | **348** |
| `herb_functions_chenoweth.md` | 舊 | 2282 | **201** | 0 | 0 |
| 同上 | **現行** | 1011 | **0** | **2079** | 0 |

舊版:`Huáng Qín (黄芩) [Skullcap root]` —— 有中文、有聲調、一欄一行。
現行:`Hu?ng Q?n () [Skullcap root]` —— 中文**全部消失**(348 個空括號就是它們
原本的位置)、聲調變亂碼、四欄壓平成一行。

四欄壓平正是先前 `herb.nan_sha_shen` 那句「該頁欄位交錯,WM 與劑量段無法安全歸屬」
的來源 —— 那不是課件本來的問題,是這次重抽造成的。

**所以錨點壞掉只是症狀。** 而且更麻煩的是:19 筆超界看得出來,另外 **28 筆仍落在
新檔範圍內、卻指到不相干段落**,任何掃描都看不出來。`pair.lu_dou__gan_cao` 就是
這型 —— 它的 `#L1245-L1247` 在**舊檔**是「Minimize toxicity: prescribe with /
Gan Cao & Lu Dou」,**當初引對了**,是重抽把它弄壞的。

## 為什麼沒照裁定做行號重映射

因為有更好的做法,而且它不依賴課件檔的處置結論:

**頁碼在兩版之間完全一致**(mm 41 頁、herb_functions 13 頁,新舊皆同)。
所以 `#pN` 跨抽取版本通用,`#L` 綁死單一次抽取、已經死過一次。
把行錨點**轉成頁錨點**比重算行號好:重抽再發生也不會壞,而且正是課件檔頭自己寫的
引用格式(`Cite as …chenoweth.pdf#p<N>`)。代價是精度變粗,但在四欄壓平的版面上
行號本來就是假精度。

只是這步該不該做、以及**課件檔本身要不要還原成舊版抽取**,是 `curriculum/**` 的
處置,那是 Ting 的目錄(AI 只讀)。兩條路的差別很大:

- 若**還原舊抽取**:那 47 筆錨點(含看不見的 28 筆)大多自動變回正確,中文也回來;
  代價是我這兩批新寫的 `#p2/#p3/#p38` 不受影響(頁錨點通用),但需要重跑 build。
- 若**維持現行檔**:47 筆要逐筆轉頁錨點,而且中文與可歸屬性回不來。
