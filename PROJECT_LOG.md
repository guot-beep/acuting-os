# 2026-08-24 Antigravity — Task 0 / Batch 3 (止血藥 + 補虛藥陰氣 55 味中藥卡補全 & 具名來源禁忌症)

- **做了什麼**: 完成 Task 0 (Batch 3) 之 55 味中藥卡（止血藥 20 味、補陰藥 18 味、補氣藥 17 味）之欄位補全。依 `docs/ANTIGRAVITY_HANDOFF.md` Task 0 規範，成對對齊 `modern_functions_zh` 與 `modern_functions_en`（純英文正則斷言 E10 全數通過，無任何 CJK 混入）；經 `curriculum/herbs/`（Chenoweth 課件: `materia_medica_abbreviated_chenoweth.md` 等）與《臺灣中藥典第四版》查證，補齊具名來源之 `contraindications_zh`，未查獲具名出處者堅決留空，絕不憑空捏造。未碰 `condition_tags_en`、`actions_en`、`cautions_zh`。
- **數字 before→after**:
  - Task 0 Batch 3 目標 55 味中藥卡處理完成。
  - 全庫 `contraindications_zh`: `126 → 179 / 363` (35% → 49%，淨增 +53 筆具名來源禁忌症)
