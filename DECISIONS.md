...
## D32 — UI 執行凍結變成閘門(9/01 生效,gate 提前上線熱身) · LOCKED(2026-08-28,執行 ROADMAP/W1-4,無新裁定)

- **What**:`scripts/validate-ui-freeze.js` 進 CI。自 2026-09-01 起,動到
  `app.js` / `js/**` / `index.html` / `styles.css` 的分支,commit 訊息必須
  自稱修 bug(修 bug/fix/修正/紅燈/回歸/hotfix)或明寫「凍結例外」+理由。
  凍結日之前是報告模式(永遠 exit 0),讓大家先習慣它的存在。
- **Why**:凍結保護的不是「程式碼完美」,是 **Ting 對介面的肌肉記憶**。
  9/02 開診,那一週她在真實診間用這個殼 —— 開診第三天按鈕換位置,比按鈕
  醜三個月嚴重得多。而 UI 表面有 29,075 行、三天內被改了 14 次,靠自律
  擋不住:2026-08-28 夜裡 12 個 commit 動了渲染層,其中一個讓 main 紅了
  (G1/D31 兩個常數落在 TDZ)直到隔天才被發現。
- **誠實聲明制,不是防駭**:想繞過只要多寫一個字。它擋的是「順手改一下」
  那種無意識改動 —— 被迫在 commit 訊息裡寫下「這是 bug fix」的那一秒,
  人會重新想一次這到底是不是 bug。刻意不做白名單路徑或 diff 大小門檻:
  那些擋得住手滑,擋不住判斷失誤,而後者才是凍結真正要防的。
- **負控 3/3**:凍結後動 UI+訊息像新功能→擋;同樣改動但自稱修 bug→放行;
...
> **附記(2026-09-02 凌晨,工程狀態)**:Ting 當晚追加「今天直接做」並授權「接管電腦、七小時」。程式在分支 `claude/d1-worker`
