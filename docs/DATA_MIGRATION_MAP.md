...
| CloudTCM formula indication taxonomy (2473) | `data/herbs/cloudtcm_formula_indication_tags.json`; English authority in `data/config/cloudtcm_formula_indication_en.json` | NOT wired into app; translation queue only | CloudTCM `/formula` `pageDataList_2` + curated English overrides | SOURCE VOCABULARY 2026-07-22; 2473/2473 Chinese labels and direct routes, English 58/2473 curated draft; must not render as a complete bilingual layer until reviewed |
...
| WHO acupoint location staging | `data/imports/acupoint_sources/who_location_staging.json` | `scripts/extract-who-acupoint-locations.py` and `scripts/preview-who-cun-fill.js`; NOT wired into app | WHO 2008 location standard, temporary PDF not committed | REVIEW-ONLY; 361 extracted locations and 100 fill-empty B-cun proposals; no apply mode, canonical `361.json` unchanged |
| Acupoint anatomy and high-risk safety staging | `data/imports/acupoint_anatomy/*.json` | `scripts/preview-acupoint-anatomy-review.js`, `scripts/preview-acupoint-anatomy-fill.js`, `scripts/preview-acupoint-protocol-anatomy.js`; NOT wired into app | Peer-reviewed ultrasound/MRI/cadaver/anatomy and human-study protocol tables | REVIEW-ONLY; high-risk preview has 34 fields / 38 values; protocol-table preview adds 8 fields / 12 values and withholds LR3 conflict; no apply mode, canonical `361.json` unchanged |
