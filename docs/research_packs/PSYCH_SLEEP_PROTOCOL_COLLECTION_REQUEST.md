...
- `restless_legs`:**這是神經科診斷,不是精神科** ——
  ...
  收集何時應轉神經科。
- `somatic_symptom`:~~前提是器質病因已被適當排除~~ ——
  **2026-08-14 更正(SOL 於 B3 交付中指出,他是對的)**:DSM-5-TR 的邏輯不是這樣。
  SSD **不是**「查不到病因才給的診斷」,它可以與已確診的身體疾病並存;
  診斷依據是身體症狀帶來的過度想法、感受與行為,不是「排除器質病因」。
  原本那句話寫反了,而且危險 —— 它會讓人以為貼上 SSD 標籤就等於不必再查。
  請在 scope 寫明:**SSD 可與真正的醫療疾病並存,且不得以本診斷為由停止必要檢查**。
