...
### 5n. SOL 第二輪查源裁定判例(2026-08-14,Ting 授權執行)

SOL 對 B10 全庫掃尾扣住的 6 張 safety hold(yin_chen_hao、mo_yao、huang_bai、san_leng、
shi_hu、fo_shou)逐句查源後執行,本輪總原則:

- **古籍禁忌不得自動補現代機制**:批次生成流程常在「不宜服/忌服」等古籍/傳統結論後方
  自動掛上一段現代機轉解釋(如「因其具有活血化瘀作用」「因其含有生物鹼」)，這段解釋
  多半查無出處、甚至與該記錄自己的 `modern_functions_zh`/`properties_taste_temp` 方向
  相反。查源後只保留查得到來源的結論句，機轉解釋句整段封存刪除，不代寫新的機轉。
- **原典等級(不宜/忌/禁)不得升降格**:古籍原文寫「不宜用」就譯 Not suitable / Avoid，
  寫「忌用/禁用」才譯 Contraindicated；不可因為白話改寫或現代機制描述的語氣而把
  「不宜」抬高成「禁用」，也不可把「禁用」降格成「慎用」(呼應 §3a 嚴重度階梯鐵則)。
- **preclinical 訊號寫（體外）標註,不寫臨床口吻**:`modern_functions_zh/_en` 若來源僅為
  體外/前臨床研究(如佛手 in vitro 藥理)，中文加註「（體外）」、英文加註「(in vitro)」，
  不得寫成暗示臨床療效的裸詞(例:「降血糖」→「糖代謝調節活性（體外）」/
  "Glucose-metabolism-modulating activity (in vitro)")；若該項訊號與人體研究方向相反
  (如佛手降血壓一項，人體吸入研究反而觀察到血壓上升)，整項封存刪除，不做（體外）標註
  掩護,因為掩護不了方向相反的結論。

**太子參(herb.tai_zi_shen)evidence_level 註記**:cautions_zh[3]/[4] 上游 CloudTCM 原頁
標題「高血壓」「糖尿病」與內文「低血壓」「低血糖」方向相反，疑為生成模板套錯高/低變體
(B8 判例已標記同類問題)。本輪僅修正表頭語義為「低血壓／正在使用降壓藥物者」「低血糖／
正在使用降糖藥物者」，不代改內文、不升級為 verified clinical contraindication —— 這兩則
的 evidence level 維持 preclinical/weak，翻譯與後續引用都不得把它讀成臨床實證的禁忌。
