...
## ⚡ NEXT(2026-09-09,Claude 派工):Task 12R-v2(取代下面的 12R)—— 你 4 天沒動,舊範圍已經是九牛一毛

**先講狀況**:下面 09-05 派的 12R,你從頭到尾沒碰過——`CODEX_HANDOFF.md` 最後一條是 09-04
(Task 11H),`codex/` 系列分支最後一次推送也是 09-04。這 4 天 main 又疊了將近 80 個 commit
(`git rev-list --count 410f7308..origin/main` = 79):五天派工四包(A 搜尋／B 手機排版／
C 渲染成本／D 卡片語意)、D10–D14 一串裁定、D33 通行碼認證整條改版、CI 棘輪閘門修復…
舊 12R 鎖定的 19 個 commit 現在只是這一大段的開頭。**角色不變,你還是覆核員,只出報告不改資料**;
下面的規格(逐項要做的事、報告格式、完成定義)全部照舊沿用,唯一改的是範圍跟優先序。

### MEASURED TREE

`origin/main @ add40c4a`。開工前自己 `git fetch origin main` 重算一次:
```bash
git rev-list --count b196248f..origin/main    # 應為 98
```
對不上先回報,不要沿用這裡寫死的數字硬做。

### 範圍分三個優先序桶——時間有限,照順序做,做多少誠實回報(不要為了湊涵蓋率跳著做)

**P0(沒有第二個人覆核過,風險最高,一定要做)**

1. **09-09 兩個 hashchange/rAF 時序 bug 修正**(`b8f7f1c8..add40c4a`,兩個 commit:
   鑑別表 `e22b7f9c` + 病症卡 `add40c4a`)——單人 session,PROJECT_LOG 沒有列對抗式審查者。
   commit 訊息自己承認「Browser pane 不跑 requestAnimationFrame,人眼沒看過 flash 動畫本身,
   證明的只是程式路徑」——這句話本身就是你要覆核的重點:程式路徑證明是否真的成立,
   有沒有可能程式路徑對但實際瀏覽器行為不對。
2. **D33 通行碼認證**(7 個具名 commit,不是連續區段——中間穿插其他線的批次:
   `22491d62`、`86ac4609`、`9397646a`、`e7eb17bd`、`d077c595`、`de1c84ea`、`ea15ea0f`;
   `git log b196248f..origin/main --oneline --grep="通行碼\|D33" -i` 可重現這份清單):
   拿掉 Cloudflare Access、改存雜湊在 D1、git 只放一次性設定碼——這是安全機制整條換掉,
   沒看到獨立第二人覆核記錄。重點查:git 歷史(不只工作樹)裡有沒有殘留低熵密碼材料、
   雜湊是不是真的只存在 D1(grep 整個 repo 找有沒有明碼或弱雜湊漏網)、canary 腳本是否真的
   會在通行碼機制壞掉時 fail(故意改壞一次跑一遍)。
3. **CI 棘輪閘門修復**(`df204009`、`1f6d8656`):commit 訊息自己寫「9/02 起 15 層裡有一層在
   CI 上是暗的,沒人發現」。**這件事本身就是理由去懷疑「這次修完真的會亮嗎」**——故意在這個
   分支上引入一個明知該被擋下的迴歸(隨便一種既有 fixture 都可以),推一個假 commit 觸發
   `validate.yml`,確認新流程真的變紅、而不是又暗掉一次。

**P1(已有內部對抗式審查,但影響大,值得第三方再核一輪,抽樣即可、不必全覆核)**

4. 五天派工 09-07 四包(A/B/C/D,`b196248f..b8f7f1c8` 這段):PROJECT_LOG 已經記了
   「執行 → 審查」配對(Fable→Opus、Opus→Opus、Sonnet→Sonnet、Sonnet/Fable→Sonnet),
   但審查者跟執行者在同一輪次裡,不是真正獨立的第三方。**每包抽 3-5 條 commit 訊息裡的
   具體數字重跑一次**,抽中就深查,沒抽中的不用管。

**P2(餘力才做)**

5. 其餘 commit(D10 系列裁定執行、D5 穴位圖候選寫入、藥用部位分批、encoding 分類等):
   至少把 commit 訊息裡「N 條」「N/M」這種宣稱挑幾個重跑,不用逐檔讀 diff。
...
一份報告,每條發現都附可重現的指令;沒有「大致」「應該」「看起來」。P0 三項是底線,
P1/P2 做多少算多少、誠實回報覆蓋了幾分之幾,不要用「完成」「100%」。

---

## （已由上方 12R-v2 取代，你從未動工，留著給下一個人看範圍怎麼長大的）Task 12R —— **只做檢查,不做填寫**:對抗式覆核 Claude 落地的每一批
