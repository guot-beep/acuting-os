...
## 八、待辦 · 針灸體系分頁（2026-07-31，Claude token 用盡未做）

### 8.1 把「穴位 index」提升為同級按鈕

現在的體系按鈕列在 `index.html:319` 的 `#systemTabsBar`，共 7 個 `.system-tab-btn`：
...
**Ting 的要求**：目前「穴位 index」藏在「十四正經」底下的第二層。請把它**提升成與董氏奇穴同級的獨立按鈕**，放進同一列，樣式與其他按鈕一致（`.system-tab-btn` + `.sys-svg` 圖示 + `data-mode-text` 雙語 span）。

**不要**把它繼續留在十四正經的下拉層。

### 8.2 每個按鈕的第二層要有功能

每個體系按鈕點下去之後的第二層（子篩選），要照 Ting 截圖的樣式做成可用的功能列，不是只有標籤。截圖顯示的是經外奇穴選中時，底下出現 `⭐ 特定穴類別`、`🩺 臨床主題與證型`、`📍 身體部位` 這類子篩選。

### 8.3 ⚠️ 做這件事之前必讀

**針灸目錄的版面目前是壞的，而且我修錯過一次。**

已確認的事實：

- `#cards` 在 1280px 視窗下算出來是 **380px 單欄**，769 張卡擠成一直排
- `.cards` 的規則本身正確：`repeat(auto-fill, minmax(240px, 1fr))`
- 上層 `.content-grid.acupoint-browser` 是 `380px 513px` 兩欄，**卡片列表在窄的那格**
- `styles.css:3607` 的 `.content-grid { grid-template-columns: minmax(300px, 380px) minmax(0, 1fr); }` 是相關規則

**我試過把兩欄對調（`minmax(0,1fr) minmax(260px,340px)`），量出來列表變 553px、兩欄排列——數字看起來對，但 Ting 說畫面更糟，已 revert（`19bbad6`）。**

所以問題不只是欄寬，可能牽涉 `.details-panel` 或地圖面板的定位。

**教訓（我犯了四次）**：卡片數量、`render()` 有沒有拋錯、grid 算出幾欄——**這些全部是綠燈，版面照樣是壞的**。改版面**一定要開瀏覽器看畫面**，用 `computer{action:"screenshot"}`，不能只量數字。Ting 說的一直是外觀，我量的一直是功能。

### 8.4 未推送的 commit

`origin/main` 停在 `635dbef`，本機還有：

```
19bbad6 Revert 版面改動
48ca449 版面改動（已被還原）
f94a3da overlay 機制  ← 這個有價值,務必推上去
```

推送一直 HTTP 408 失敗。請重試 `git push origin main`。
