...
1b. **真資料形狀排練(2026-08-14 晚補充)**:對備份檔的**隔離副本**跑
   `node scripts/rehearse-c2b.js <pre-c2b副本.json>`(Codex P3.2 既有腳本:
   完整週期 + 冪等 0/0/0 + rollback + 原始位元組不變斷言)。
   **此步全綠才准碰瀏覽器**——它驗證的是 Ting 真實資料的形狀,
   Phase R 的虛構資料驗證不了這一點。副本用畢即刪,不進 repo。
