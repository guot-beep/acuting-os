...
- 私人使用:個人學習收集,無版權疑慮;病例一律去識別化(改名字/patient_code),無隱私疑慮。
- **acuting.com 是另一回事**:那是未來診所的公開網站,跟本系統完全分開;本系統永遠私人。
...
- 部署:**同一個 AcuTing OS** 放上 Cloudflare Pages + Access 密碼(私人網址,手機可用)。與公開的 acuting.com 無關。
...
**中長期(病例↔知識圖譜層)**:Ting 2026-07-29 定的方向 —
病人/病程/看診三層 hub、雙向連結(「SP6 用過 18 例」)、追蹤指標詞彙表、
臨床反思六問、複習佇列七來源、搜尋涵蓋病例層,以及新的評估權重
(資料結構 20% / 搜尋 18% / 病例知識連結 18% / 視覺 4% / SEO≈0)。
規格見 **`docs/CLINICAL_GRAPH_TRACK.md`**;它不改本文件的架構定案。
...
2. ~~新站名~~(誤解已澄清:網站就叫 AcuTing OS,不改名;「改名字」指病例去識別化)
