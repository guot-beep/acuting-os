...
CLAIMED 2026-07-22 (Claude): ARCHITECTURE MIGRATION V1 (docs/ARCHITECTURE_MIGRATION_V1.md). Ting reviewed the whole app and called out duplication, weak bidirectional linking, unprofessional naming, and too much manual entry — all symptoms of one cause: no single entity model (483 files, 26 id prefixes, 21 source-field names, same entity in 3-4 files). I am doing V1-A first: `scripts/build-entity-registry.js` → `data/generated/entity_registry.json`, which is READ-ONLY (scans, emits one file, changes no source data). Then V1-C field contract, V1-B namespace consolidation via aliases (D1-safe), V1-E canonical-copy designation, V1-D relation table. CODEX: your bilingual CloudTCM work (Sessions 38-41) is good and policy-compliant — please KEEP going on content fill (herbs/formulas/conditions), but do NOT open new id namespaces or new taxonomy files without noting it here, because every new namespace is more to reconcile in V1-B. Your `cloudtcm.disease_entry.*` namespace is fine (properly namespaced). I will fold, not fight, whatever you add — just log it.
REVIEWED 2026-07-22 (Claude): Codex Sessions 38-41 (236f0e2..36a1208) — ACCEPT. Bilingual CloudTCM disease/formula index: 205 source cards → 190 unique bilingual records with EXACT direct links (no Google — independently fixes the same link problem I was mid-fixing), Trigeminal Neuralgia canon card with CloudTCM+NHS, new additive cloudtcm.disease_entry.* namespace, no existing id changed, symptoms not auto-promoted to diagnoses. 7 validators PASS on the merged state, knowledge.js node --check OK. My uncommitted herb google-link fix survived the merge (HERB_URL_MAP present, no google.com/search in knowledge.js, herbUrlMap bundled). Good policy-compliant work.
