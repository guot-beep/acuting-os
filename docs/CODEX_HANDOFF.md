...
- **14 full-content cards**: `cond.chronic_diarrhea`, `cond.nausea_vomiting`, `cond.hemorrhoids`, `cond.chronic_constipation`, `cond.stroke_rehab`, `cond.diabetic_neuropathy`, `cond.dizziness_vertigo`, `cond.cinv`, `cond.food_sensitivity`, `cond.postherpetic_neuralgia`, `cond.essential_tremor`, `cond.gallbladder_dysfunction`, `cond.post_op_ileus`, `cond.migraine_vestibular`. Added bilingual summary/context/risk/red flags/acupuncture scope, direct official URLs, field_sources, registry refs where available, and valid `sym.*` relations. Existing non-empty canonical fields were never overwritten by the fill tool.
- **6 provenance-audited emergency cards**: `cond.acute_angle_closure_glaucoma`, `cond.cauda_equina_syndrome`, `cond.guillain_barre_syndrome`, `cond.myasthenia_gravis`, `cond.retinal_detachment`, `cond.preeclampsia`. Existing content was cross-checked against NEI/NINDS/NICHD/CDC/MedlinePlus/NHS sources; only direct URLs and field-level provenance were added.
- **Measured delta**: maturity `151→171 FULL_DETAIL_CANDIDATE`, `351→334 DETAIL_PARTIAL`, `3→0 SKELETON`; direct source entries `553→632`; records with sources `241→245/505`; records with field_sources `465→474/505`; bilingual red flags `491→505/505`; bilingual acupuncture scope `413→427/505`; records with sign_symptom_ids `118→131/505`.
- **Current Conditions gap map**: the remaining 334 partial records have overlapping missing dimensions: etiology 279, sources 260, acupuncture_scope 78, risk_factors 52, summary 42, western_context 42, field_sources 31, structured_relations 26, western_pathology 21. These are not additive counts. Red flags are now present bilingually on all 505 records.
- **Do not silently certify two audited old claims**: `cond.appendicitis` cites NIDDK for “age 10–30” and family history, while the current NIDDK page supports higher frequency in teens/20s but does not support that exact range or family-history claim. `cond.bowel_obstruction` uses NIDDK intestinal pseudo-obstruction as inline provenance for several mechanical-obstruction claims. No canonical deletion or overwrite was made because that requires Ting approval; these are the next source-correction decisions.
- **Read-only adjacent inventory**: 361 regular points are 361/361 template-grade; extra points are 50/72 strict complete with 22 needing work. Formulas are 216/223 template-grade, 15 have bilingual misalignment, and the correctness validator retains one error (`四神丸` name/count heuristic) plus one gap (`甘麥大棗湯` no chief role). Symptoms are 102/102 validator-clean. This Conditions branch did not modify those datasets.
- **Validation**: `node scripts/build-data.js`; `validate-condition-standard --json`=`505 clean / 0 defects`; `check-validation-ratchet` PASS; `validate-condition-sources` PASS (`632` direct entries); `validate-relation-registry` PASS; `validate-content-junk` PASS with frozen existing warnings only; `git diff --check` PASS. Also ran read-only `validate-acupoint-standard`, `validate-extra-point-standard`, `validate-formula-standard`, `validate-formula-correctness`, `validate-formula-quality-strict`, and `validate-symptom-standard` for the inventory above.
