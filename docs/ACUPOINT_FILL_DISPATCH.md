...
→ KI(27,注意 11 穴的 `cautions_zh` 是英文要修)→ PC(9)→ TE(23,**TE3 中渚是考點**)
→ GB(44)→ LV(14)→ CV(24)→ GV(28)

**已完成**:LU、LI、ST、SP(97/361 穴到樣板)。
每一經一支腳本(`scripts/curate-<經絡>-channel.js`),中文層寫在腳本裡、
英文一律由 `parse-channel-curriculum.py` 讀出,腳本自己 assert 中英條數對齊
與「英文每個字都出現在該穴的課件原文裡」,對不上就拒絕寫入。

⚠️ TE 那批注意:課件的考試星號**通常在代碼上**(`SP-6**`),但 TE3 與 SP21
的星號在拼音上(`Zhong Zhu*`、`Da Bao*`)。解析器已修成兩邊都讀,
但 `TE3` 目前 `exam_star` 還是 undefined —— 做 TE 時要一併補。
