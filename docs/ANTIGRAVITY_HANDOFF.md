...
Codex 做了（`docs/CODEX_TASK_QUEUE.md` 有完整派工單）。**如果你已經在做、或分支已經在推了，
不用停，讓我知道就好**——只是不要兩邊重做同一件事，同一份清單有兩份互相打架的版本更難處理。
下面原本的 Task 11G 說明留著給你參考，不用重讀。

---

## 📋 新派工 Task 11G（2026-08-28，已改派 Codex，見上）——董氏穴位死連結的**處置清單**（只出清單，不改資料）
...
`--verify-ledger --scope bundle` 用**重新產生的 bundle 檔**（不信任已 commit 的）比對，5596/5596
精確吻合。mastertungacupuncture.org 那個發現自己重現了：722 張圖真 404、251 個內容頁真 200，
是圖片目錄搬走了不是整站掛掉。

**上一輪留的那個「PubMed 需要有網路的第三方查證」懸案，我用 PubMed E-utilities API 查完了**：
雄黃兩個 PMID（41270212、36110533）論文真實存在，主題對得上砷毒性，帳本引文跟真摘要逐字吻合。
不是 Task 5 那種造假，可以結案。你不用再花時間處理這個懸案。
...
真 404 不是 soft-404)、11C(95 筆中藥引用網址補齊，80 填入/31 誠實留空，覆蓋率 73%→95% /
95%→96%)。11C 抽查 3 筆(2 FILLED + 1 NO_SOURCE_FOUND)都對得上，方法論(`how_found` 索引頁點入、
不猜拼音)確實照規矩做。細節跟數字見 `PROJECT_LOG.md` 對應條目。

**複核過程中我自己一度誤判**：用派工單當時寫的舊 `MEASURED TREE` SHA 跑 `--verify-fill`，把
另一批(考綱歸屬核對)在你開工之後才落地的正當修正，錯認成你動了不該動的 `key_pairs` 欄位。
改用 rebase 後的正確 parent 重跑就全部 PASS——這不是你的問題，是我複核時該用「當下」基準沒有
沿用「派工時」基準，寫這裡備查，以後複核這種快速推進的任務會記得先重算分母再判定。
...
- **核心數據 (SSOT 直出)**:
  - Base SHA: `3ced3b32d029ed98f54c8e58a9995e18e186afc8` (origin/main)
  - Audit Source SHA: `3ced3b32d029ed98f54c8e58a9995e18e186afc8`
...
## 🚩 巡檢簡記:`task10d-evidence-provenance-fragmentation` 開了——你自己 round4 寫的「不開始 Task 10D」沒守住(2026-08-27)

推在分支上,沒推 main,這點照舊做對了;純新增(新工具+新報告+新 JSON+log/handoff 條目,0 正典資料
異動),跟前四條線同一個模式。**這次特別記一筆,不是因為又擴大範圍**(那個已經連續提醒過四次了)**,
是因為你自己在 round4 的「下一步」寫的是「推送至 round4,等待 Ting/團隊審閱,不開始 Task 10D」,
結果 30 分鐘後就開了 Task 10D**。這不是單純自我擴大範圍,是講了會做的事跟實際做的事不一致——這件
事本身比任何一條稽核線的內容都重要,麻煩注意一下自己在報告裡寫的「下一步」要算數。這次一樣不深入
複核,先簡記。
