...
## ⚠️ Task 7 不採用（`afd3a69f`）——自我驗證通過,但驗證的不是這次要抓的問題

你交回的報告自我驗證 10/10 樣本吻合，但**這 10 個樣本剛好全部都是「`contraindications_en` 欄位是
空的」這一種型態**（5 張缺欄位判定為「有問題」、5 張沒缺欄位判定為「合格」）——完全沒有一個樣本
是「這句翻譯翻錯了」或「英文讀不通」。這代表你的自我驗證只證明了「我能正確數出哪些卡缺一個欄位」，
沒有證明「我能分辨語意翻對還是翻錯」——而後者才是這個任務真正要做的事。

**結果**：全庫報告 219 筆「發現」，我逐條核對過，**438 個問題描述句裡沒有一句提到翻錯、翻反、讀
不通、亂碼、或內容對不上這味藥**，100% 都是「中文有內容、英文缺失」這個單一模式。我拿
`node scripts/validate-herb-standard.js` 直接核對，這個 219 **跟驗證器自動報出來的
「contraindications_en missing on 219 record(s)」完全一樣的數字**——這份報告沒有提供任何驗證器
本來就沒有的新資訊，等於做了任務裡最容易的那一半（數空格），完全沒碰真正要求的那一半（讀卡抓語意
錯誤）。這份報告不採用，不會進 main。

**重做規則**：自我驗證的 10 張樣本裡，**至少要有 3-4 張是「內容已經翻對、沒有欄位缺漏」的卡**（不
是隨便選，是刻意選幾張你確定翻得好的卡），確認你的判斷邏輯在這幾張上正確回報「沒問題」，而不是
只在「缺欄位」的卡上驗證。如果你的方法本質上只能檢查「欄位有沒有值」，那這個方法**不適合**這個任務
——這個任務要的是讀懂中英文內容本身、判斷語意是否對應，不是欄位存在性檢查（存在性檢查已經有
`validate-herb-standard.js` 在做，不需要你重做）。真的找不到語意層的錯誤也沒關係，那就在報告裡誠實
寫「逐卡讀過 N 張，語意層沒有發現真的錯誤」，不要用「缺欄位」的清單充數。

**其他規則不變**：唯讀、不改 `herb_canon_shortlist.json`、輸出到
`docs/audits/HERB_SEMANTIC_QA_<日期>.md`、推到獨立分支等驗收。
