...
## 🔥 Task 7：中藥卡語意品質稽核（唯讀，不寫 herb_canon_shortlist.json；Task 0 已收工，現在解鎖）

Task 6 那兩批寫入型任務（`related_formulas`/`formula_family`）目前先擱著，沒有更可靠的做法之前不
硬做——這條線改派這個**唯讀**任務，風險低很多（不寫資料，只交報告）。

**範圍**：`main` 上現有 **363** 味中藥卡（`data/herbs/herb_canon_shortlist.json`），全部，不限分類。

**背景**：`validate-herb-standard.js` 的 E10 能抓「整條中文完全沒翻譯、直接複製貼上」這種明顯錯誤，
但抓不到「翻了、但翻錯了」或「翻譯本身讀不通」這種語意層問題——那個只能靠人讀卡。**這個任務之前
你做過一次**（`docs/audits/HERB_SEMANTIC_QA_2026-08-21.md`），標了 226/358 味「有問題」，被我整份
不採用——抽查發現檢查邏輯本身壞了，產生大量假陽性（例如中文「陰虛血熱者慎用」對應英文已經寫
「Use cautiously in Yin deficiency with Blood Heat」，報告卻說「英文缺乏警示詞」；中文「補陽」
對應英文「Tonifies Yang」，報告卻說「缺乏補益動詞」——`Tonifies` 本身就是 `Tonify` 的變位，檢查
邏輯沒有正確處理動詞變位）。**這次重做,先過這一關再跑全庫**：

**做之前先自我驗證（這次的鐵律，跟上次不一樣的地方）**：
1. 手動挑 10 張卡（自己選，涵蓋不同分類），對照 `functions_zh`/`modern_functions_zh`/`cautions_zh`
   跟對應的 `_en`，人工判斷這 10 張各自有沒有語意問題。
2. 拿你打算用的檢查方法（不管是關鍵字比對、還是別的邏輯）跑這 10 張，把結果跟人工判斷對照。
3. **如果檢查邏輯對這 10 張的判斷跟人工判斷對不上（尤其是誤判「沒問題」為「有問題」），先把邏輯
   修好、重新跑這 10 張確認對得上，再跑全庫 363 張**——不要邏輯還沒驗證過就直接跑全庫產報告。
4. 在報告開頭寫一段「自我驗證」：這 10 張是哪幾張、人工判斷 vs 邏輯判斷的對照結果。
...
**輸出**：新增一份新檔案 `docs/audits/HERB_SEMANTIC_QA_2026-08-25.md`（`docs/audits/` 資料夾不存在就
新建）。開頭附上面的自我驗證段落。每一條問題寫：`herb.<id>`（藥名）、欄位名、中文原文、目前的英文
翻譯、你認為的問題、建議修法（不需要真的改，寫建議就好）。沒問題的卡不用寫，只列有問題的。

**驗證**：做完後 `git status` 應該只多出這一份新檔案（跟自我驗證段落），`data/` 底下完全零異動——
這條是唯讀稽核，不是填補。推到 `antigravity/herb-semantic-qa-task7` 獨立分支。

**驗收**：我會挑幾條報告裡標的「有問題」自己核對是不是真的有問題（比照你的自我驗證方法反過來查我），
過了才採用。
