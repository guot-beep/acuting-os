...
## ⏳ Task 9A Round 3：中藥 + 方劑 來源傳輸完整性審計（待驗收）

- **類型**: READ-ONLY Canonical Provenance & Transport Integrity Audit（正典與生成檔 0 異動）
- **分支**: `antigravity/task9a-source-integrity-audit-round3`
...
- **Tokenizer & Invariant 強化**:
  - 內建 13 項回歸測試套件 (100% PASS)，完整支援檔名空格、Unicode 標點 (–)、中文檔名、行號與頁碼錨點 (#p2, #L102, #L282-L312) 及尾端中文註解。
  - 嚴格落實 HTTP 分類閉合不變量：`sum(all HTTP classifications) === uniqueHttpUrls` (1260/1260)。
  - 嚴格以 repo root 為基準檢驗本地路徑存在性。
- **摘要數據 (SSOT 直出)**:
  - 唯一 HTTP URL 探測: 1,260 條（OK_200: 1,084, REDIRECT_TO_200: 3, DEAD_4XX: 95, SERVER_5XX: 73, TIMEOUT: 3, OTHER_HTTP_STATUS: 2）
  - 本地路徑引用: 6,181 處（LOCAL_EXISTS: 5,473, LOCAL_MISSING: 708，唯一路徑 95 條）
  - Malformed URL: 0 處
