...
## ⏳ Task 9B Round 2：中藥 + 方劑 重複/棄用/殘根/孤立引用盤點（待驗收）

- **類型**: READ-ONLY Canonical Integrity Inventory（正典與生成檔 0 異動）
- **分支**: `antigravity/task9b-canonical-duplicate-orphan-audit-round2`
...
- **Round 2 升級點**:
  - 修復 Whitespace / Unicode 標點 Normalization 正則，嚴格區分 `EXACT_ID_DUPLICATE`、`CASE_NORMALIZED_ID_COLLISION`、`WHITESPACE_NORMALIZED_ID_COLLISION`。
  - 內建 6 項啟動回歸測試套件 (100% PASS)。
  - `loadJsonStrict()` 杜絕 malformed JSON 靜默 swallow，遇錯誤必中斷 (Audit Fail)。
  - 區分引用來源狀態，僅當 active source reference > 0 時標註 `DEPRECATED_BUT_REFERENCED_BY_ACTIVE`。
- **核心數據 (SSOT 直出)**:
  - 掃描記錄: Herbs 363 / Formulas 223 (共 586 筆)
  - Deprecated 記錄: 8 筆（Herbs 4 筆, Formulas 4 筆；其中 1 筆 `formula.bai_du_san` 仍有 4 處活躍主方引用 `DEPRECATED_BUT_REFERENCED_BY_ACTIVE`，其餘 7 筆為 `UNREFERENCED_DEPRECATED`）
  ...
  - 結構化引用掃描: 3,784 處（`TARGET_EXISTS_ACTIVE`: 3,775, `TARGET_EXISTS_DEPRECATED`: 4, `TARGET_MISSING`: 5）
