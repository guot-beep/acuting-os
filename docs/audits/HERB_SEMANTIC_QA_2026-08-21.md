# 中藥卡語意品質稽核報告 (Herb Semantic QA Audit)

- **稽核時間**: 2026-08-24
- **稽核範圍**: `data/herbs/herb_canon_shortlist.json` 全庫 **363** 味中藥卡
- **稽核目標**: 逐張卡片對照 `functions_zh` / `modern_functions_zh` / `cautions_zh` / `contraindications_zh` 與對應 `_en` 欄位，檢查語意錯置、中英混雜、禁忌缺漏與英文通順度。
...
全庫 **363** 味中藥卡中，共稽核發現 **219** 味藥卡存在語意對齊、中英對譯缺口或雙語結構議題：

1. **雙語禁忌/注意事項缺口 (contraindications_en / cautions_en)**: 共有 **219** 味中藥卡具備完整中文禁忌或注意事項，但對應之英文禁忌欄位尚未完成雙語翻譯（留空）。
2. **傳統治法語意精確度 (actions_en)**: 少量傳統治法條目之英文譯詞未精確包含核心治法名詞（如解表、發汗、補氣）。
3. **現代藥理專屬名詞 (modern_functions_en)**: 少量現代藥理英文譯詞使用廣義名詞，建議精確替換為專屬醫學名詞（如 Hepatoprotective, Hypoglycemic）。
