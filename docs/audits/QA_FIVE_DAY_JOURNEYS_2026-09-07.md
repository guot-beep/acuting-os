...
- 卡片高亮(`.gr-flash`)靠 `requestAnimationFrame`,Browser pane 隱藏時不會跑,本次沒能在自動化裡驗到它;要人眼看一次。
  → **2026-09-09 已收**:自動化驗不到的原因之外,這條路以前根本沒閃過(鑑別卡缺 `data-record-id`;hashchange 是任務不是同步;router 的區塊捲動蓋掉卡片捲動),先修 e22b7f9c(鑑別表)與 add40c4a(病症),再由 Ting 人眼驗鑑別表與病症各一張,兩張都閃。
