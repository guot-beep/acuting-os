...
> **Execution Date**: 2026-08-26T05:38:58.694Z  
...
| **Canonical Duplicate & Orphan Gate (Task 9B)** | **PASS WITH WARNINGS** | 0 | 0 | 1 (5 TARGET_MISSING orphan references preserved) | 0 |
| **Generated Sync & Sandbox Rebuild Gate (Task 9C)** | **PASS WITH WARNINGS** | 0 | 0 | 1 (`entity_registry.json` differs) | 0 |
| **Validator Taxonomy & CI Coverage Gate (Task 9C)** | **PASS WITH WARNINGS** | 0 | 0 | 1 (12 orphan blocking validators preserved) | 0 |
| **Source & Provenance Transport Gate (Task 9A)** | **PASS WITH WARNINGS** | 0 | 0 | 2 (5 missing local paths; 95 dead HTTP links baseline) | 0 |
...
(12 In-Memory Fixtures)
    C -- "12/12 PASS" --> D["Fast / Deterministic Mode
...
(UTF-8, C0 Controls, Strict JSON Syntax)"]
    D --> G["3. Canonical Gate (Task 9B)
(Exact Duplicates, Whitespace/Case Collisions, 5 Orphan Refs)"]
    D --> H["4. Generated Gate (Task 9C)
(7+1 Sync Domains & Sandbox Deterministic Rebuild)"]
    D --> I["5. Validator & CI Gate (Task 9C)
(Taxonomy, Blocking/Informational, False-Green Detection)"]
    D --> J["6. Source Gate (Task 9A)
(Anti-Bare-Tail Local Path Tokenizer)"]
...
1. **Fast / Deterministic Mode (`node scripts/antigravity-preflight.js`)**:
   - Deterministic local checks only (zero network calls, execution time ~3s, default 100% read-only).
...
## 3. Identity-Aware Debt Ratchet & Baseline Comparison
...
| **Unique Missing Local Paths** | 5 unique paths | 5 unique paths | `KNOWN_DEBT_PASS` (0 regressions, 0 additions) |
