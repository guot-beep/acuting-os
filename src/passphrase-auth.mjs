...
/* 迭代次數的取捨(2026-09-02 在本機 workerd 量過:30,000 次的登入請求整趟 73 ms):
 * Workers 免費方案是**每個請求 10 ms CPU**,慢雜湊在這裡不能無限加。而慢雜湊真正防的是
 * 「雜湊被偷走之後離線暴力破解」—— 要偷到它得先進她的 Cloudflare 帳號,而那時 D1 也一起沒了。
 * 線上猜測則由「加權長度 ≥16 的通行碼 + 每 15 分鐘 8 次」擋住,跟迭代數無關。
 * 所以預設 30,000:登入(每台裝置每 30 天一次)不會撞 CPU 上限。升級 Workers Paid 之後可以往上調。 */
