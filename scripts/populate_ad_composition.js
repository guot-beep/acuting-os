...
// Herb pinyin to zh / en / id / pinyin_toned map
const pinyinMap = new Map();

// Dictionary overrides for processed herbs, minerals, and special forms
...
// Helper for Chinese translation of action
function translateActionToZh(enAction) {
  if (!enAction) return '本方組成藥物之一';
  let zh = enAction;
  
  if (enAction.includes('Strong Qi tonic') || enAction.includes('Tonifies Qi')) zh = '補氣健脾，充沛宗氣與中氣';
  else if (enAction.includes('dries Dampness') && enAction.includes('Spleen')) zh = '健脾燥濕，運化水濕';
  else if (enAction.includes('Drains Dampness') || enAction.includes('drains Damp')) zh = '利水滲濕，健脾寧心';
  else if (enAction.includes('Harmonizes') || enAction.includes('harmonizes')) zh = '調和諸藥，緩和藥性，益氣和中';
  else if (enAction.includes('Moves Qi') || enAction.includes('moves Qi')) zh = '行氣寬中，理氣化痰';
  else if (enAction.includes('descends rebellious Qi') || enAction.includes('stops vomiting')) zh = '降逆止嘔，燥濕化痰';
  else if (enAction.includes('Nourishes Yin') || enAction.includes('nourishes Yin')) zh = '養陰潤肺，益胃生津';
  else if (enAction.includes('Nourishes Blood') || enAction.includes('nourishes Blood')) zh = '補血養血，和營調經';
  else if (enAction.includes('Warms') || enAction.includes('warms')) zh = '溫經散寒，通絡止痛';
  else if (enAction.includes('Releases exterior') || enAction.includes('releases exterior')) zh = '發汗解表，宣肺平喘';
  else {
    zh = enAction
      .replace(/Tonifies Qi/gi, '補氣')
      .replace(/strengthens Spleen/gi, '健脾')
      .replace(/dries Dampness/gi, '燥濕')
      .replace(/drains Dampness/gi, '利濕')
      .replace(/harmonizes/gi, '調和')
      .replace(/nourishes Blood/gi, '養血')
      .replace(/nourishes Yin/gi, '養陰')
      .replace(/clears Heat/gi, '清熱')
      .replace(/moves Qi/gi, '理氣')
...
blocks.forEach(block => {
  ...
  const headerLine = lines[0]; // e.g. Si Jun Zi Tang | 四君子湯 | Four Gentlemen Decoction
  ...
  lines.forEach(l => {
    if (l.startsWith('|') && !l.includes('Pharmaceutical Latin') && !l.includes('---')) {
      ...
      if (cols.length >= 5) {
        ...
        let name_zh = matchObj.zh || cleanPinyin;
        let name_en = matchObj.en || cleanLatin;
        ...
        composition.push({
          ...
          in_formula_en: actionsRaw, // 100% exact copy from AD MD file
          actions_en: actionsRaw,    // 100% exact copy from AD MD file
          in_formula_zh: translateActionToZh(actionsRaw), // clean concise Chinese
          role_reason_zh: translateActionToZh(actionsRaw),
...
console.log(`Successfully populated composition for all ${blocks.length} formulas (${updatedCount} updated, ${createdCount} created) in formulas.json.`);
