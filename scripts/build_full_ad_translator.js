...
function translateAction(en) {
  if (!en) return '';

  // Direct exact phrase matches
  const directMap = {
    'Tonifies Qi': '補益氣血',
  ...
  if (directMap[en]) return directMap[en];

  // Natural rule-based component replacement
  let res = en
    .replace(/Tonifies/gi, '補益')
    .replace(/Strengthens/gi, '健旺')
    .replace(/Harmonizes/gi, '調和')
    .replace(/Clears/gi, '清瀉')
    .replace(/Nourishes Yin/gi, '滋陰養陰')
    .replace(/Nourishes Blood/gi, '補血養血')
    .replace(/Nourishes/gi, '滋陰')
    .replace(/Cools/gi, '涼血')
    .replace(/Warms/gi, '溫煦')
    .replace(/Stops/gi, '止')
    .replace(/Transforms/gi, '化')
    .replace(/Dries/gi, '燥')
    .replace(/Expels/gi, '祛')
    .replace(/Relieves/gi, '緩解')
    .replace(/Promotes/gi, '促進')
    .replace(/Reduces/gi, '平復')
    .replace(/Resolves/gi, '化解')
    .replace(/Astringes/gi, '固澀')
    .replace(/Moistens/gi, '潤澤')
    .replace(/Unblocks/gi, '通利')
    .replace(/Invigorates/gi, '活化')
    .replace(/Disperses/gi, '消散')
    .replace(/Vents/gi, '透達')
    .replace(/Spleen/gi, '脾')
    ...
    .replace(/Kidney/gi, '腎')
    .replace(/Liver/gi, '肝')
    .replace(/Blood Stasis/gi, '瘀血')
    .replace(/Blood/gi, '血')
    .replace(/Qi/gi, '氣')
    .replace(/Yin/gi, '陰')
    .replace(/Yang/gi, '陽')
    .replace(/Damp-Heat/gi, '濕熱')
    .replace(/Dampness/gi, '濕邪')
    .replace(/Damp/gi, '濕')
    .replace(/Deficiency Heat/gi, '虛熱')
    .replace(/Heat/gi, '熱')
    .replace(/Cold/gi, '寒')
    .replace(/Phlegm/gi, '痰')
    .replace(/Wind/gi, '風')
    .replace(/Exterior/gi, '表邪')
    .replace(/Interior/gi, '裏邪')
    .replace(/sweating/gi, '汗出')
    .replace(/sweat/gi, '汗')
    .replace(/cough/gi, '咳嗽')
    .replace(/wheezing/gi, '喘促')
    .replace(/pain/gi, '疼痛')
    .replace(/vomiting/gi, '嘔吐')
    .replace(/diarrhea/gi, '泄瀉')
    .replace(/prolapsed organs/gi, '臟器脫垂')
    .replace(/Middle Jiao/gi, '中焦')
    .replace(/Lower Jiao/gi, '下焦')
    .replace(/Upper Jiao/gi, '上焦')
    .replace(/the/gi, '')
    .replace(/and/gi, '與')
    .replace(/or/gi, '或')
    .replace(/with/gi, '兼');
...
function translateSyndrome(en) {
  ...
  let res = en
    ...
    .replace(/Kidney/gi, '腎')
    .replace(/Liver/gi, '肝')
    .replace(/Gallbladder/gi, '膽')
    .replace(/Blood/gi, '血')
    .replace(/Qi/gi, '氣')
    .replace(/Yin/gi, '陰')
    .replace(/Yang/gi, '陽')
    .replace(/Damp-Heat/gi, '濕熱')
    .replace(/Dampness/gi, '濕')
    .replace(/Damp/gi, '濕')
    .replace(/Heat/gi, '熱')
    .replace(/Cold/gi, '寒')
    .replace(/Phlegm/gi, '痰')
    ...
    .replace(/body/gi, '體')
    .replace(/usually/gi, '')
    .replace(/later stages/gi, '後期')
    .replace(/Warm-Heat/gi, '溫熱')
    .replace(/pathogen/gi, '邪')
    .replace(/lurking/gi, '伏')
    .replace(/aspects/gi, '');

  let cleaned = res.replace(/\s+/g, '').replace(/[()]/g, '').trim();
  if (!cleaned.endsWith('證')) cleaned += '證';
  return cleaned;
}

// Ingest Actions & Syndromes from American_Dragon_201_Formulas_Name_Actions_Syndromes.md
...
blocks.forEach(block => {
  ...
  if (target) {
    if (actionsEn.length) {
      ...
      target.actions_zh = actionsEn.map(translateAction);
    }
    if (syndromesEn.length) {
      target.ad_syndromes_en = syndromesEn;
      target.pattern_indications_en = syndromesEn;
      target.pattern_indications_zh = syndromesEn.map(translateSyndrome);
...
console.log(`Successfully rebuilt clean bilingual Actions and Syndromes for ${updatedCount} / ${blocks.length} formulas in formulas.json.`);
