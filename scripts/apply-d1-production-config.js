...
(function main() {
  ...
  if (REVERT) {
    ...
    cfg.vars = { ENVIRONMENT: "production", CLINICAL_DB_NAME: "acuting-clinical", ACCESS_TEAM_DOMAIN: team, ACCESS_AUD: aud, ACCESS_ALLOWED_EMAILS: emails };
    writeWrangler(cfg, HEADER_PROD);
    fs.writeFileSync(INDEX, setMeta(fs.readFileSync(INDEX, "utf8"), true));
    console.log("wrangler.jsonc 與 index.html 已寫成正式(D1)設定。");
  }
  ...
  console.log(REVERT ? "\n下一步:git diff 看一眼 → commit(訊息含「回滾」)→ push main。"
    : "\n下一步:確認無痕視窗開 https://acuting-os.guotingru.workers.dev/ 會被要求 Access 登入 → git diff → commit(訊息含「D33 切換」)→ push main → 等 Workers Builds 部署 → 照 runbook 核對五點。");
})();
