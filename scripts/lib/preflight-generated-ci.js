...
function getRuntimeLoadedFiles(root) {
    ...
    for (const m of scriptMatches) {
      ...
      if (!src.startsWith('http://') && !src.startsWith('https://')) {
        ...
        if (!fs.existsSync(path.join(root, src))) {
          missingFromDisk.push({ html: hf, reference: src });
        }
      }
    }
    ...
    for (const m of linkMatches) {
      ...
      if (!href.startsWith('http://') && !href.startsWith('https://') && !href.startsWith('data:')) {
        ...
        if (!fs.existsSync(path.join(root, href))) {
          missingFromDisk.push({ html: hf, reference: href });
        }
      }
    }
  });

  return { loadedFiles: Array.from(loaded), missingFromDisk };
}

function buildDependencyGraph(root) {
  const runtimeInfo = getRuntimeLoadedFiles(root);
  const runtimeLoadedSet = new Set(runtimeInfo.loadedFiles);
  ...
  generatedArtifacts.forEach(rel => {
    let status = 'GENERATED_BUT_UNUSED';
    if (runtimeLoadedSet.has(rel)) {
      status = 'DIRECT_RUNTIME_LOADED';
    } else if (rel === 'data/quality/content_quality.json') {
      // Transitively read by build-data.js and bundled into knowledge_core.js which is in runtimeLoadedSet
    ...
    classifications.push({
      artifact: rel,
      status,
      isRuntimeLoaded: status !== 'GENERATED_BUT_UNUSED'
    });
  });

  return {
    runtimeLoadedFiles: runtimeInfo.loadedFiles,
