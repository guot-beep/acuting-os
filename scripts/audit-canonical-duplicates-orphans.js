...
// Strict JSON Loader (Fails loudly on malformed JSON)
...
function auditEntities(records, entityType) {
  ...
  records.forEach((r, idx) => {
    const rawId = String(r.id !== undefined ? r.id : (r.code !== undefined ? r.code : `idx_${idx}`));
    const lowerId = rawId.toLowerCase();
    const wsId = rawId.replace(/\s+/g, '');

    // 1. Exact ID tracking
    if (!seenExactIds.has(rawId)) seenExactIds.set(rawId, []);
    seenExactIds.get(rawId).push(idx);

    // 2. Case-only collision tracking
    if (!seenCaseIds.has(lowerId)) seenCaseIds.set(lowerId, []);
    seenCaseIds.get(lowerId).push(rawId);

    // 3. Whitespace-normalized collision tracking
    if (!seenWsIds.has(wsId)) seenWsIds.set(wsId, []);
    seenWsIds.get(wsId).push(rawId);

    const isDep = r.review_status === 'deprecated';
    const isStub = rawId.includes('_import_stub') || (r.name_zh && r.name_zh.includes('匯入重複殘根'));
  ...
  for (const [id, indices] of seenExactIds.entries()) {
    if (indices.length > 1) {
      duplicateIdExact.push({
        type: 'EXACT_ID_DUPLICATE',
        id,
  ...
  for (const [lowerId, ids] of seenCaseIds.entries()) {
    const uniqueRawIds = Array.from(new Set(ids));
    if (uniqueRawIds.length > 1) {
      duplicateIdCase.push({
        type: 'CASE_NORMALIZED_ID_COLLISION',
        normalizedId: lowerId,
        collidingIds: uniqueRawIds
      });
    }
  }
  ...
  for (const [wsId, ids] of seenWsIds.entries()) {
    const uniqueRawIds = Array.from(new Set(ids));
    if (uniqueRawIds.length > 1) {
      duplicateIdWhitespace.push({
        type: 'WHITESPACE_NORMALIZED_ID_COLLISION',
        normalizedId: wsId,
        collidingIds: uniqueRawIds
      });
    }
...
// Startup Regression Test Suite
function runStartupRegressionTests() {
  console.log('Running Task 9B Regression Test Suite...');

  // Test 1: Whitespace collision (trailing space vs no space)
  const test1 = auditEntities([{ id: 'formula.foo' }, { id: 'formula.foo ' }], 'formula');
  if (test1.duplicateIdWhitespace.length !== 1 || test1.duplicateIdWhitespace[0].type !== 'WHITESPACE_NORMALIZED_ID_COLLISION') {
    throw new Error('Regression Test 1 Failed: Whitespace ID collision not detected');
  }

  // Test 2: Whitespace-normalized interior space collision
  const test2 = auditEntities([{ id: 'formula.foo' }, { id: 'formula. foo' }], 'formula');
  if (test2.duplicateIdWhitespace.length !== 1 || test2.duplicateIdWhitespace[0].type !== 'WHITESPACE_NORMALIZED_ID_COLLISION') {
    throw new Error('Regression Test 2 Failed: Interior whitespace collision not detected');
  }

  // Test 3: Case-normalized collision
  const test3 = auditEntities([{ id: 'formula.foo' }, { id: 'formula.Foo' }], 'formula');
  if (test3.duplicateIdCase.length !== 1 || test3.duplicateIdCase[0].type !== 'CASE_NORMALIZED_ID_COLLISION') {
    throw new Error('Regression Test 3 Failed: Case collision not detected');
  }

  // Test 4: Chinese name whitespace normalization
  if (normZh('四 妙 丸') !== '四妙丸' || normZh('四 妙 丸') !== normZh('四妙丸')) {
    throw new Error('Regression Test 4 Failed: Chinese name whitespace normalization failed');
  }

  // Test 5: Chinese punctuation normalization
  if (normZh('都氣丸(加減)') !== '都氣丸加減' || normZh('都氣丸-加減') !== '都氣丸加減' || normZh('都氣丸，加減') !== '都氣丸加減') {
    throw new Error('Regression Test 5 Failed: Chinese punctuation normalization failed');
  }

  // Test 6: Malformed JSON parser fails loudly
  let failedLoudly = false;
  try {
    const badJsonStr = '{"broken": [1, 2, }';
    JSON.parse(badJsonStr);
  } catch (e) {
    failedLoudly = true;
  }
  if (!failedLoudly) {
    throw new Error('Regression Test 6 Failed: Malformed JSON did not throw');
  }

  console.log('Task 9B Regression Test Suite: 6/6 tests passed.');
}

runStartupRegressionTests();

// 1. Load Canonical Herbs and Formulas
const herbsData = loadJsonStrict(HERBS_FILE);
const formulasData = loadJsonStrict(FORMULAS_FILE);
...
function recordRef(sourceFile, sourceRecordId, fieldPath, referencedId, refEntityType, sourceStatus = 'active') {
  ...
  allStructuredReferences.push({
    ...
    sourceStatus, // 'active' | 'deprecated' | 'import_stub'
    fieldPath,
...
formulas.forEach(f => {
  ...
  if (Array.isArray(f.composition)) {
    f.composition.forEach((c, idx) => {
      if (c && c.herb_id) recordRef(FORMULAS_FILE, fId, `composition[${idx}].herb_id`, c.herb_id, 'herb', sourceStatus);
    });
  }
  if (Array.isArray(f.related_formulas)) {
    f.related_formulas.forEach((rf, idx) => {
      ...
      if (refId) recordRef(FORMULAS_FILE, fId, `related_formulas[${idx}]`, refId, 'formula', sourceStatus);
    });
  }
  if (Array.isArray(f.formula_family)) {
    f.formula_family.forEach((ff, idx) => {
      const refId = typeof ff === 'string' ? ff : (ff && (ff.formula_id || ff.id));
      if (refId) recordRef(FORMULAS_FILE, fId, `formula_family[${idx}]`, refId, 'formula', sourceStatus);
    });
  }
  if (f.single_herb_id) {
    recordRef(FORMULAS_FILE, fId, 'single_herb_id', f.single_herb_id, 'herb', sourceStatus);
  }
});
...
herbs.forEach(h => {
  ...
  if (Array.isArray(h.related_formulas)) {
    h.related_formulas.forEach((rf, idx) => {
      const refId = typeof rf === 'string' ? rf : (rf && (rf.formula_id || rf.id));
      if (refId) recordRef(HERBS_FILE, hId, `related_formulas[${idx}]`, refId, 'formula', sourceStatus);
    });
  }
  if (Array.isArray(h.herb_pairs)) {
    h.herb_pairs.forEach((hp, idx) => {
      if (hp && hp.herb_id) recordRef(HERBS_FILE, hId, `herb_pairs[${idx}].herb_id`, hp.herb_id, 'herb', sourceStatus);
      if (hp && hp.pair_herb_id) recordRef(HERBS_FILE, hId, `herb_pairs[${idx}].pair_herb_id`, hp.pair_herb_id, 'herb', sourceStatus);
    });
  }
  if (Array.isArray(h.key_pairs)) {
    h.key_pairs.forEach((kp, idx) => {
      if (kp && kp.herb_id) recordRef(HERBS_FILE, hId, `key_pairs[${idx}].herb_id`, kp.herb_id, 'herb', sourceStatus);
    });
  }
});
...
function enrichDepStubList(list) {
  return list.map(item => {
    ...
    const activeRefs = refs.filter(r => r.sourceStatus === 'active');
    const depRefs = refs.filter(r => r.sourceStatus === 'deprecated');
    const stubRefs = refs.filter(r => r.sourceStatus === 'import_stub');

    let riskClassification = 'UNREFERENCED_DEPRECATED';
    if (activeRefs.length > 0) {
      riskClassification = 'DEPRECATED_BUT_REFERENCED_BY_ACTIVE';
    } else if (refs.length > 0) {
      riskClassification = 'DEPRECATED_REFERENCED_ONLY_BY_DEPRECATED';
    }

    return {
      ...item,
      incomingReferencesCount: refs.length,
      incomingActiveReferencesCount: activeRefs.length,
      incomingDeprecatedReferencesCount: depRefs.length,
      incomingImportStubReferencesCount: stubRefs.length,
      isReferencedByActive: activeRefs.length > 0,
      riskClassification,
...
enrichedHerbDeprecated.concat(enrichedFormulaDeprecated).forEach(d => {
  if (d.incomingActiveReferencesCount > 0) {
    highestRiskCandidates.push({
      riskType: 'DEPRECATED_BUT_REFERENCED_BY_ACTIVE',
      severity: 'HIGH',
      recordIds: [d.recordId],
      entityType: d.entityType,
      detail: `Deprecated record ${d.recordId} is referenced by ${d.incomingActiveReferencesCount} active records/fields.`
    });
  }
});
...
const herbSummary = {
  ...
  deprecatedReferencedByActiveCount: enrichedHerbDeprecated.filter(d => d.incomingActiveReferencesCount > 0).length
};
...
const formulaSummary = {
  ...
  deprecatedReferencedByActiveCount: enrichedFormulaDeprecated.filter(d => d.incomingActiveReferencesCount > 0).length
};
...
const overallSummary = {
  ...
  totalDeprecatedButReferencedByActive: herbSummary.deprecatedReferencedByActiveCount + formulaSummary.deprecatedReferencedByActiveCount,
...
const fullAuditReport = {
  ...
  audit_round: 'Round 2',
  audit_name: 'Canonical Duplicate, Deprecated, Stub & Orphan Reference Inventory (Task 9B Round 2)',
  scope: {
...
const mdLines = [
  ...
  `| **DEPRECATED_BUT_REFERENCED_BY_ACTIVE Records** | ${herbSummary.deprecatedReferencedByActiveCount} | ${formulaSummary.deprecatedReferencedByActiveCount} | ${overallSummary.totalDeprecatedButReferencedByActive} |`,
  '',
  '---',
  '',
