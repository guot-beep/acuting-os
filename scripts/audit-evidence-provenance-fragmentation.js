#!/usr/bin/env node
/**
 * audit-evidence-provenance-fragmentation.js — Task 10D Round 4.1: Path-Preserving Generated Summary
 *
...
function computeFieldAggregateStatus(datasetResults) {
  const statuses = new Set(datasetResults.map((r) => r.status));
  if (statuses.size === 1) {
    const s = Array.from(statuses)[0];
    if (s === "SURVIVES_VERBATIM") return "ALL_SURVIVE_VERBATIM";
    if (s === "DROPPED") return "ALL_DROPPED";
    if (s === "NOT_BUNDLED") return "ALL_NOT_BUNDLED";
    return s;
  }
  return "MIXED";
}

function evaluateGeneratedSurvival(canonicalOccurrences, canonicalRecordsMap, runtimeCollections) {
  ...
  // Evaluate each occurrence individually (per dataset + field path)
  const detailedResults = [];
  const fieldOccurrencesGrouped = new Map();

  for (const occ of canonicalOccurrences) {
    const { field_name, field_path, dataset_id } = occ;
    const records = canonicalRecordsMap.get(dataset_id) || [];
    const genRecords = datasetToGeneratedMap.get(dataset_id);

    const res = evaluateDatasetSurvivalDeep(records, genRecords, field_path);
    const item = {
      field_name,
      field_path,
      dataset: dataset_id,
      status: res.status,
      records_compared: res.records_compared,
      evidence: res.evidence
    };
    detailedResults.push(item);

    if (!fieldOccurrencesGrouped.has(field_name)) {
      fieldOccurrencesGrouped.set(field_name, []);
    }
    fieldOccurrencesGrouped.get(field_name).push(item);
  }

  // Field-level aggregate
  const fieldAggregates = [];
  for (const [fieldName, items] of fieldOccurrencesGrouped.entries()) {
    const aggregateStatus = computeFieldAggregateStatus(items);
    fieldAggregates.push({
      field_name: fieldName,
      aggregate_status: aggregateStatus,
      datasets_count: items.length,
      surviving_datasets: items.filter((i) => i.status === "SURVIVES_VERBATIM").map((i) => i.dataset),
      dropped_datasets: items.filter((i) => i.status === "DROPPED").map((i) => i.dataset),
      transformed_datasets: items.filter((i) => i.status === "TRANSFORMED").map((i) => i.dataset),
      unbundled_datasets: items.filter((i) => i.status === "NOT_BUNDLED").map((i) => i.dataset)
    });
  }

  return {
    per_dataset_occurrences: detailedResults,
    field_level_aggregates: fieldAggregates.sort((a, b) => a.field_name.localeCompare(b.field_name))
  };
}
...
function runAudit() {
        ...
        for (const [k, v] of Object.entries(obj)) {
          ...
          if (cat !== SEMANTIC_CATEGORIES.UNKNOWN_OR_AMBIGUOUS) {
            ...
            if (cat !== SEMANTIC_CATEGORIES.CANDIDATE_RELATED_FIELD) {
              ...
              // Record occurrence by dataset + field_name + exact field_path
              const occKey = `${d.id}::${k}::${fullKey}`;
              if (!seenOccurrenceKeys.has(occKey)) {
                seenOccurrenceKeys.add(occKey);
  ...
  // Generated Data Behavior (Evaluated per dataset + field path & aggregated)
  const generatedDataBehavior = evaluateGeneratedSurvival(canonicalOccurrences, canonicalRecordsMap, runtimeCollections);

  return {
    meta: {
      timestamp: "2026-08-27T01:48:00Z",
      base_sha: baseSha,
...
function runSelfTest() {
  console.log("=== RUNNING TASK 10D ROUND 4.1 REGRESSION SUITE (8 FIXTURES) ===");
  let passed = 0;
  ...
  // Fixture 8: comprehensive generated survival assertions across deep records, nested paths, cross-dataset isolation & aggregate status
  ...
  // 8.5 Cross-dataset same field name -> no contamination (A preserves, B drops -> aggregate MIXED)
  ...
  const resA = evaluateDatasetSurvivalDeep(canA, genA, "field_sources");
  const resB = evaluateDatasetSurvivalDeep(canB, genB, "field_sources");
  assert.strictEqual(resA.status, "SURVIVES_VERBATIM");
  assert.strictEqual(resB.status, "DROPPED");
  assert.strictEqual(computeFieldAggregateStatus([resA, resB]), "MIXED");
  ...
  // 8.7 Real repo verification of nested path condition_canon.acupoint_protocol_evidence.protocol_status
  const condCanonPath = path.join(ROOT, "data/pathology/condition_canon_shortlist.json");
  if (fs.existsSync(condCanonPath)) {
    const cRecords = JSON.parse(fs.readFileSync(condCanonPath, "utf8")).records || [];
    let nestedCount = 0;
    for (const r of cRecords) {
      if (getNestedValue(r, "acupoint_protocol_evidence.protocol_status") !== undefined) {
        nestedCount++;
      }
    }
    assert.ok(nestedCount > 0, "acupoint_protocol_evidence.protocol_status must be present in condition_canon canonical records");
  }

  console.log("PASS [Fixture 8]: Deep record survival, value equality, nested paths, cross-dataset isolation, and aggregate MIXED verified");
  passed++;
...
function generateMarkdownReport(auditData) {
  ...
  const genBehavior = auditData.generated_data_behavior;

  return `# Evidence & Provenance Fragmentation Inventory — Task 10D (Round 4.1)
...
|---|---|---|---|---|---|
${genBehavior.field_level_aggregates.map((fa) => `| \`${fa.field_name}\` | \`${fa.aggregate_status}\` | ${fa.datasets_count} | ${fa.surviving_datasets.map((d) => `\`${d}\``).join(", ") || "無"} | ${fa.dropped_datasets.map((d) => `\`${d}\``).join(", ") || "無"} | ${fa.unbundled_datasets.map((d) => `\`${d}\``).join(", ") || "無"} |`).join("\n")}

### B. 資料集與欄位路徑細部明細（Per-Dataset & Field-Path Details）
| 欄位名稱 | 欄位路徑 (Field Path) | 資料庫 (Dataset) | 存活狀態 (Status) | 比對筆數 | 觀察依據 (Evidence) |
|---|---|---|---|---|---|
${genBehavior.per_dataset_occurrences.map((g) => `| \`${g.field_name}\` | \`${g.field_path}\` | \`${g.dataset}\` | \`${g.status}\` | ${g.records_compared} | ${g.evidence} |`).join("\n")}
...
function main() {
  ...
  if (args.includes("--write-report")) {
    ...
    console.log("\n================================================================================");
    console.log("       ACUTING OS EVIDENCE & PROVENANCE FRAGMENTATION INVENTORY (ROUND 4.1)     ");
    console.log("================================================================================");
