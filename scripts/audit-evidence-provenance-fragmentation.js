#!/usr/bin/env node
/**
 * audit-evidence-provenance-fragmentation.js — Task 10D Round 3: Final Measurement Integrity
 *
 ...
 *
 * Core Invariant:
 *   "A property name is not a fallback operand, and a field name appearing in a generated file is not proof that the canonical field survived into runtime."
 *
 * Features:
...
const CANONICAL_DATASETS = [
  { id: "herbs", path: "data/herbs/herb_canon_shortlist.json", family: "herbs", is_bundled: true },
  { id: "herb_pairs", path: "data/herbs/herb_pairs.json", family: "herbs", is_bundled: false },
  { id: "formulas", path: "data/herbs/formulas.json", family: "formulas", is_bundled: true },
  { id: "conditions", path: "data/pathology/conditions.json", family: "biomedical_conditions", is_bundled: true },
  { id: "condition_canon", path: "data/pathology/condition_canon_shortlist.json", family: "biomedical_conditions", is_bundled: true },
  { id: "tdis_registry", path: "data/pathology/tdis_registry.json", family: "tcm_diseases", is_bundled: true },
  { id: "cloudtcm_diseases", path: "data/pathology/cloudtcm_disease_entries.json", family: "tcm_diseases", is_bundled: false },
  { id: "pattern_library", path: "data/pathology/pattern_library.json", family: "patterns", is_bundled: true },
  { id: "pattern_registry", path: "data/pathology/pattern_registry.json", family: "patterns", is_bundled: true },
  { id: "acupoints_361", path: "data/acupoints/361.json", family: "acupoints", is_bundled: true },
  { id: "extra_points", path: "data/acupoints/extra_points.json", family: "acupoints", is_bundled: true },
  { id: "auricular_points", path: "data/auricular/embedded/auricular_points.json", family: "acupoints", is_bundled: true },
  { id: "scalp_points", path: "data/scalp/scalp_points_full.json", family: "acupoints", is_bundled: true },
  { id: "tung_points", path: "data/tung/point_index.json", family: "acupoints", is_bundled: true },
  { id: "symptoms", path: "data/symptoms/symptoms.json", family: "symptoms", is_bundled: true },
  { id: "comparisons", path: "data/knowledge/comparisons.json", family: "comparisons", is_bundled: true },
  { id: "red_flag_registry", path: "data/pathology/red_flag_registry.json", family: "safety", is_bundled: true },
  { id: "formula_safety_flags", path: "data/herbs/formula_safety_flags.json", family: "safety", is_bundled: false },
  { id: "pharm_drugs", path: "data/pharmacology/drugs.json", family: "pharmacology", is_bundled: true },
  { id: "pharm_drug_classes", path: "data/pharmacology/drug_classes.json", family: "pharmacology", is_bundled: true },
  { id: "western_medications", path: "data/medications/western_medications.json", family: "pharmacology", is_bundled: false },
  { id: "supplements", path: "data/supplements/supplements.json", family: "supplements", is_bundled: true },
  { id: "outcome_metrics", path: "data/clinical_cases/outcome_metrics.json", family: "clinical_config", is_bundled: false },
  { id: "avs_advice_library", path: "data/config/avs_advice_library.json", family: "clinical_config", is_bundled: false },
  { id: "content_quality", path: "data/quality/content_quality.json", family: "quality", is_bundled: false },
  { id: "formula_hdi_review", path: "data/quality/formula_hdi_review.json", family: "quality", is_bundled: false },
  { id: "source_registry", path: "data/sources/source_registry.json", family: "source_registry", is_bundled: false }
];
...
// Build-Graph Generated Survival Evaluator
function discoverRuntimeLoadedGeneratedArtifacts() {
...
function evaluateDatasetSurvival(canonicalRecords, generatedFieldMap, fieldName, isBundledDataset) {
  if (!isBundledDataset) {
    return {
      status: "NOT_BUNDLED",
  ...
  for (const cRec of canonicalRecords) {
    ...
    if (fieldName in cRec && cRec[fieldName] !== null && cRec[fieldName] !== undefined && cRec[fieldName] !== "") {
      totalSampled++;
    }
  }
  ...
  if (totalSampled === 0) {
    return {
      status: "NOT_BUNDLED",
      evidence: "No non-empty canonical records observed for field"
    };
  }

  const inGenerated = generatedFieldMap.has(fieldName);
  if (inGenerated) {
    return {
      status: "SURVIVES_VERBATIM",
      evidence: `Preserved in runtime bundles across ${totalSampled} canonical records`
    };
  }

  return {
    status: "DROPPED",
    evidence: `Field omitted from runtime generated records (present in ${totalSampled} canonical records)`
  };
}

function evaluateGeneratedSurvival(provenanceFields, canonicalRecordsMap) {
  const loadedArtifacts = discoverRuntimeLoadedGeneratedArtifacts();
  const generatedFieldMap = new Set();

  // Load generated fields from runtime-loaded artifacts
  for (const rel of loadedArtifacts) {
  ...
  for (const f of provenanceFields) {
    ...
    for (const d of CANONICAL_DATASETS) {
      ...
      const hasField = records.some((r) => r && f in r && r[f] !== null && r[f] !== "");
      if (!hasField) continue;

      const res = evaluateDatasetSurvival(records, generatedFieldMap, f, d.is_bundled);
...
function runAudit() {
  ...
  const generatedDataBehavior = evaluateGeneratedSurvival(Array.from(discoveredProvenanceFieldsSet), canonicalRecordsMap);

  return {
    meta: {
      timestamp: "2026-08-27T01:32:00Z",
      base_sha: baseSha,
...
function runSelfTest() {
  console.log("=== RUNNING TASK 10D ROUND 3 REGRESSION SUITE (8 FIXTURES) ===");
  let passed = 0;
  ...
  // Fixture 8: deterministic synthetic canonical -> generated survival asserts exact expected status
  const testCan = [
    { id: "herb_1", review_status: "verified", internal_note: "secret_1" },
    { id: "herb_2", review_status: "draft", internal_note: "secret_2" }
  ];
  const testGenMap = new Set(["review_status"]);

  const s1 = evaluateDatasetSurvival(testCan, testGenMap, "review_status", true);
  const s2 = evaluateDatasetSurvival(testCan, testGenMap, "internal_note", true);
  const s3 = evaluateDatasetSurvival(testCan, testGenMap, "review_status", false);

  assert.strictEqual(s1.status, "SURVIVES_VERBATIM");
  assert.strictEqual(s2.status, "DROPPED");
  assert.strictEqual(s3.status, "NOT_BUNDLED");
  console.log("PASS [Fixture 8]: Generated survival asserts exact expected classifications (SURVIVES_VERBATIM, DROPPED, NOT_BUNDLED)");
  passed++;
...
function generateMarkdownReport(auditData) {
  ...
  return `# Evidence & Provenance Fragmentation Inventory — Task 10D (Round 3)
...
function main() {
  ...
  if (args.includes("--write-report")) {
    ...
    console.log("\n================================================================================");
    console.log("       ACUTING OS EVIDENCE & PROVENANCE FRAGMENTATION INVENTORY (ROUND 3)       ");
    console.log("================================================================================");
