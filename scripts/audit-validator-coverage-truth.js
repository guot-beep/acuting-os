 ...
 *
 * AcuTing OS — Task 10B Round 4: Validator Coverage Truth Table & Guard Gap Inventory
 *
 ...
 * 2. Behavior-based connected retired-ID guard discovery (observable connected deprecated query, reference scan, fail-closed, CI tier).
...
// ── Behavior-Based Retired-ID Guard Scanner (Connected Semantics) ────────────
function findActiveRetiredIdGuards(root = ROOT, directMap = new Map(), transitiveMap = new Map(), customScriptsDict = null) {
  ...
  for (const rel of scriptFiles) {
    ...
    // 1. Behavior 1 & 3 (Connected): Collects retired IDs into a container and compares scanned values against it
    const candidateCollections = new Set(clean.matchAll(/(?:const|let|var)\s+([a-zA-Z0-9_$]+)\s*=\s*(?:new\s+(?:Map|Set)|\[\])/g).map(m => m[1]));
    let hasDeprecatedCollection = false;
    let hasComparison = false;

    for (const colName of candidateCollections) {
      const popPattern = new RegExp(`review_status\\s*===?\\s*['"]deprecated['"][\\s\\S]{0,120}\\b${colName}\\.(?:set|add|push)\\(|\\b${colName}\\.(?:set|add|push)\\([\\s\\S]{0,120}review_status\\s*===?\\s*['"]deprecated['"]`);
      if (popPattern.test(clean)) {
        if (new RegExp(`\\b${colName}\\.(?:has|includes)\\(`).test(clean)) {
          hasDeprecatedCollection = true;
          hasComparison = true;
          break;
        }
      }
    ...
    if (!hasDeprecatedCollection || !hasComparison) {
      continue;
    }

    // 2. Behavior 2: Traversal/reference scanning across records/files
    const hasDataTraversal = /(?:data[/\\\\]|data|walk|readdirSync|readFileSync)/.test(clean) &&
                             /(?:scan\(|walk\(|for\s*\([^)]+of\s+files|for\s*\([^)]+of\s+records)/.test(clean);

    // 4. Behavior 4: Exclude the deprecated record's own identity / subtree / self-reference
    const hasSelfExclusion = /(?:node\.id|record\.id|self|own)[\s\S]{0,80}(?:===|!==|==|!=|\.has\()[\s\S]{0,80}(?:deprecated|return)/.test(clean) ||
                            /(?:outside|except|subtree|self-reference)/i.test(clean) ||
                            /node\.review_status\s*===?\s*['"]deprecated['"]\s*\)?\s*(?:return|continue)/.test(clean);

    // 5. Behavior 5: Fail non-zero when references exist
    const ctrl = analyzeControlFlow(clean);
    const hasFailClosed = ctrl.hasExitNonZero;

    if (hasDataTraversal && hasSelfExclusion && hasFailClosed) {
...
function auditSpecialQuestions(root = ROOT, directMap = new Map(), transitiveMap = new Map(), customScriptsDict = null) {
  // A. Active -> Deprecated references guard discovery (Behavior-Based connected scanner)
  ...
  return {
    A_active_to_deprecated_references: {
      ...
      primary_guard: primaryGuardScript || 'NONE',
...
function generateMarkdownReport(data) {
  ...
  lines.push(`- **判定結論**：**\`${qA.guard_result}\`**（Primary Guard: \`${qA.primary_guard}\`）`);
  ...
  lines.push(`- **判定結論**：**\`${qB.guard_result}\`**（Primary Guard: \`${qB.primary_guard}\`）`);
  ...
  lines.push(`- **判定結論**：**\`${qD.guard_result}\`**（Primary Guard: \`${qD.primary_guard}\`）`);
...
// ── Synthetic Regression Test Suite (12 Fixtures) ────────────────────────────
function runSelfTest() {
  console.log('=== RUNNING TASK 10B REGRESSION FIXTURE TEST SUITE (12 FIXTURES) ===');
  let passed = 0;
  let total = 0;
...
`;
  ...
  // 10. Behavior-based guard discovery: AVS-style validator must NOT qualify
  const avsStyleScript = `
    const records = readJson("data/config/avs.json").records;
    const failures = [];
    for (const r of records) {
      if (r.review_status === "deprecated" && r.active !== false) failures.push(r.id);
    }
    if (failures.length) process.exit(1);
  `;
  const avsGuards = findActiveRetiredIdGuards(ROOT, new Map([['scripts/validate-avs-library.js', []]]), new Map(), {
    'scripts/validate-avs-library.js': avsStyleScript
  });
  assert('Production findActiveRetiredIdGuards correctly rejects AVS-style validator (deprecated -> active:false)',
    avsGuards.length === 0);

  // 11. Behavior-based guard discovery: Arbitrary-filename validator with connected behaviors qualifies
  const arbitraryGuardScript = `
    const files = walk("data");
    const retired = new Set();
    for (const f of files) {
      if (node.review_status === "deprecated") retired.add(node.id);
    }
    const violations = [];
    for (const f of files) {
      if (node.review_status === "deprecated") return; // exclude own record
      if (retired.has(node.ref)) violations.push(node.ref);
    }
    if (violations.length) process.exit(1);
  `;
  const customGuards = findActiveRetiredIdGuards(ROOT, new Map([['scripts/arbitrary-guard-name.js', []]]), new Map(), {
    'scripts/arbitrary-guard-name.js': arbitraryGuardScript
  });
  assert('Production findActiveRetiredIdGuards discovers arbitrary-filename generalized guard with connected behaviors',
    customGuards.length === 1 && customGuards[0].script === 'scripts/arbitrary-guard-name.js' && customGuards[0].ci_status === 'DIRECT_CI');

  // 12. Real repo resolution: asserts primary generalized guard resolves to validate-retired-id-references.js and NOT validate-avs-library.js
  const { directMap: realDirectMap } = parseCiWorkflows(ROOT);
  const realTransitiveMap = parseTransitiveInvocations(ROOT, realDirectMap);
  const realRepoSq = auditSpecialQuestions(ROOT, realDirectMap, realTransitiveMap);
  assert('Production auditSpecialQuestions on real repo resolves primary generalized guard to scripts/validate-retired-id-references.js (DIRECT_CI) and NOT validate-avs-library.js',
    realRepoSq.A_active_to_deprecated_references.guard_result === 'GUARD_FOUND' &&
    realRepoSq.A_active_to_deprecated_references.primary_guard === 'scripts/validate-retired-id-references.js' &&
    realRepoSq.A_active_to_deprecated_references.direct_or_transitive_ci === 'DIRECT_CI');
...
function main() {
  ...
  if (args.includes('--json')) {
    ...
    console.log('\n--- Special Questions ---');
    console.log(`A. Active -> Deprecated Refs Guard: ${data.special_questions.A_active_to_deprecated_references.guard_result} (${data.special_questions.A_active_to_deprecated_references.primary_guard})`);
    console.log(`B. D16 3 Retired Patterns Guard:    ${data.special_questions.B_d16_three_retired_patterns.guard_result} (${data.special_questions.B_d16_three_retired_patterns.primary_guard})`);
    console.log(`C. D11 Legacy Namespaces Guard:     ${data.special_questions.C_d11_legacy_diagnostic_namespaces.guard_result}`);
    console.log(`D. Retired Herb/Formula ID Guard:   ${data.special_questions.D_deprecated_herb_formula_targets.guard_result} (${data.special_questions.D_deprecated_herb_formula_targets.primary_guard})`);
    console.log('\n--------------------------------------------------------------------------------');
