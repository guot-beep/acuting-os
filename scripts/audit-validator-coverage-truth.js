 ...
 *
 * AcuTing OS — Task 10B Round 2: Validator Coverage Truth Table & Guard Gap Inventory
 *
 * READ-ONLY deterministic audit engine.
 * Fully derived truth table:
 * 1. Dynamic DECISIONS.md parser (D1..D22) + bi-directional observable code mapping.
 * 2. AST/comment-aware CI workflow and transitive invocation closure.
 * 3. Read-only execution safety analyzer (skips any script writing to data/docs).
 * 4. Separate execution status taxonomy (RED_BLOCKING, RED_TESTS, REHEARSAL_REQUIRES_ARGS, RED_REPORTS, SKIPPED_UNSAFE).
 * 5. Dynamic special-questions derivation inspecting active repo guards.
 * 6. Synthetic self-test fixtures running through production discovery pipelines.
 */
...
function isSafeToExecuteReadOnly(scriptRel, code) {
  ...
  // Check for unconditional writeFileSync to data/ or docs/ or root app files
  const writesDocs = /fs\.writeFileSync\s*\(\s*[^)]*docs[/\\\\]/.test(clean);
  const writesData = /fs\.writeFileSync\s*\(\s*[^)]*data[/\\\\]/.test(clean);
  const dangerousGit = /git\s+(?:commit|push|rebase|reset\s+--hard)/.test(clean);
  
  const hasGuardFlag = /process\.argv\.includes\(['"]--(?:write|save|fix|apply|rebaseline|migrate)['"]\)/.test(clean);

  if (dangerousGit) return false;
  if (writesDocs && !hasGuardFlag) return false;
  if (writesData && !hasGuardFlag) return false;
...
function parseTransitiveInvocations(root = ROOT, directMap = new Map()) {
  ...
  if (fs.existsSync(ratchetPath)) {
    const raw = fs.readFileSync(ratchetPath, 'utf8');
  ...
  // Also check if any direct CI script spawns or requires other scripts
...
function parseDecisionsMd(root = ROOT, customText = null, directMap = new Map(), transitiveMap = new Map()) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    // If section prose didn't mention a script but code has direct observable enforcement:
...
function auditSpecialQuestions(root = ROOT, directMap = new Map(), transitiveMap = new Map()) {
  // A. Active -> Deprecated references guard discovery
  const retiredIdGuardScript = 'scripts/validate-retired-id-references.js';
  const hasRetiredIdGuard = fs.existsSync(path.join(root, retiredIdGuardScript));
  const isRetiredIdInDirectCi = directMap.has(retiredIdGuardScript);
  const isRetiredIdInTransitiveCi = transitiveMap.has(retiredIdGuardScript);

  let qA_result = 'GUARD_NOT_FOUND';
  let qA_exists = false;
  let qA_ci = 'NONE';

  if (hasRetiredIdGuard) {
    qA_exists = true;
    if (isRetiredIdInDirectCi || isRetiredIdInTransitiveCi) {
      qA_result = 'GUARD_FOUND';
      qA_ci = isRetiredIdInDirectCi ? 'DIRECT_CI' : 'TRANSITIVE_CI';
    } else {
  ...
  let qB_result = 'GUARD_NOT_FOUND';
  let qB_behavior = '';
  if (hasRetiredIdGuard && (isRetiredIdInDirectCi || isRetiredIdInTransitiveCi)) {
    qB_result = 'GUARD_FOUND';
    qB_behavior = 'validate-retired-id-references.js (in CI) collects all review_status="deprecated" records (including D16 patterns) and fails closed on any active reference.';
  } else {
    qB_result = 'GUARD_NOT_FOUND';
  ...
  let qD_result = 'GUARD_NOT_FOUND';
  let qD_behavior = '';
  if (hasRetiredIdGuard && (isRetiredIdInDirectCi || isRetiredIdInTransitiveCi)) {
    qD_result = 'GUARD_FOUND';
    qD_behavior = 'validate-retired-id-references.js scans all JSON in data/ (including herbs and formulas) and fails closed on any reference to deprecated herb/formula IDs.';
  } else {
    qD_result = 'GUARD_NOT_FOUND';
  ...
  return {
    A_active_to_deprecated_references: {
      ...
      evidence: hasRetiredIdGuard ?
        `validate-retired-id-references.js scans all records with review_status="deprecated" and enforces 0 active references (CI: ${qA_ci}).` :
...
function runFullAudit(root = ROOT) {
  ...
  return {
    meta: {
      ...
      base_sha: '60ca605d3b66d42df7988358ea0ba6796ce560a6',
...
function generateMarkdownReport(data) {
  ...
  lines.push(`  - \`GREEN_BLOCKING_VALIDATORS\`: ${data.execution_groups.GREEN_BLOCKING_VALIDATORS.length} 支`);
...
`;
  ...
  // 2. Transitive CI validator classification (via production parser)
  const synthParentCode = `const RATCHETED = [{ script: "scripts/validate-synthetic-child.js" }];`;
  const synthCleanParent = stripComments(synthParentCode);
  const synthTransitiveMap = new Map();
  for (const m of synthCleanParent.matchAll(/script:\s*["'](scripts\/[^"']+)["']/g)) {
    synthTransitiveMap.set(m[1], 'scripts/parent.js');
  }
  assert('Transitive CI validator discovered via production parser as TRANSITIVE_CI',
    synthTransitiveMap.has('scripts/validate-synthetic-child.js') && synthTransitiveMap.get('scripts/validate-synthetic-child.js') === 'scripts/parent.js');

  // 3. Orphan blocking validator classification (via production classification)
...
`;
  ...
  // 6. Negative Fixture: child filename mentioned ONLY in comment must NOT count as transitive CI
  ...
  `;
  const cleanCommentParent = stripComments(commentOnlyParent);
  const matchesInClean = [...cleanCommentParent.matchAll(/script:\s*["'](scripts\/[^"']+)["']/g)];
  assert('Negative Fixture: Child script mentioned ONLY in comment does NOT get matched as transitive CI',
    !cleanCommentParent.includes('check-formula-no-loss.js') && matchesInClean.length === 0);
...
`;
  ...
  // 9. Read-only execution safety analyzer (via production isSafeToExecuteReadOnly)
  const unsafeScriptCode = `fs.writeFileSync(path.join(__dirname, "../docs/OUT.md"), "data");`;
  const safeScriptCode = `const doc = JSON.parse(fs.readFileSync("data/canon.json")); console.log(doc);`;
  const isUnsafe = isSafeToExecuteReadOnly('scripts/report-unsafe.js', unsafeScriptCode);
  const isSafe = isSafeToExecuteReadOnly('scripts/validate-safe.js', safeScriptCode);
  assert('Production isSafeToExecuteReadOnly flags unconditional file writer as unsafe and read-only validator as safe',
    isUnsafe === false && isSafe === true);

  // 10. Dynamic guard discovery for active -> deprecated references (via production auditSpecialQuestions)
  const realDirect = parseCiWorkflows(ROOT).directMap;
  const realTransitive = parseTransitiveInvocations(ROOT, realDirect);
  const sq = auditSpecialQuestions(ROOT, realDirect, realTransitive);
  assert('Production auditSpecialQuestions dynamically evaluates Question A and B based on active repo state',
    typeof sq.A_active_to_deprecated_references.guard_result === 'string' && typeof sq.B_d16_three_retired_patterns.guard_result === 'string');
...
function main() {
  ...
  if (args.includes('--json')) {
    ...
    console.log(`Green Blocking Validators:          ${data.execution_groups.GREEN_BLOCKING_VALIDATORS.length}`);
