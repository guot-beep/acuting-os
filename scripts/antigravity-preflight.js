 ...
 * integrity, mutation scope, generated sync, runtime consumption graph,
...
function runStartupNegativeControls() {
  ...
  // Test 7: Runtime dependency graph & transitive bundling check
  const depGraph = buildDependencyGraph(ROOT);
  const cq = depGraph.classifications.find(c => c.artifact === 'data/quality/content_quality.json');
  if (!cq || cq.status !== 'TRANSITIVELY_BUNDLED_AND_LOADED') {
    throw new Error('Negative Control 7 Failed: content_quality.json not classified as TRANSITIVELY_BUNDLED_AND_LOADED');
  }
