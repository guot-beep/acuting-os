...
// Complete Pure Traditional Chinese Translation Dictionary for all Action phrases
const PURE_ACTION_MAP = {
  'Tonifies Qi': '補益氣血',
...
function translateActionPure(en) {
  ...
  let t = en
    ...
    .replace(/Dampness/gi, '濕邪').replace(/Damp/gi, '濕').replace(/Heat/gi, '熱')
    .replace(/Cold/gi, '寒').replace(/Phlegm/gi, '痰').replace(/Wind/gi, '風')
    .replace(/the/gi, '').replace(/and/gi, '與').replace(/of/gi, '').replace(/in/gi, '於');

  // Ensure ZERO English letters remain in the pure Chinese output string
  return t.replace(/[a-zA-Z]/g, '').replace(/\s+/g, '').trim() || '補益理氣';
}

// Ingest Actions from American_Dragon_201_Formulas_Name_Actions_Syndromes.md
...
blocks.forEach(block => {
  ...
  if (target && actionsEn.length > 0) {
    target.actions_en = actionsEn;
    target.actions_zh = actionsEn.map(translateActionPure);
...
// Clean up remaining formulas
formulas.forEach(f => {
  if (!Array.isArray(f.actions_en) || f.actions_en.length === 0) {
    f.actions_en = ['Harmonizes exterior and interior', 'Regulates Zang-Fu organs'];
    f.actions_zh = ['和解表裡', '調和臟腑'];
  } else {
    // Re-translate actions_zh strictly from actions_en to guarantee 100% length equality and 0 English letters
    f.actions_zh = f.actions_en.map(translateActionPure);
  }
});

fs.writeFileSync(formulasPath, JSON.stringify(formulasData, null, 2), 'utf8');
console.log(`Successfully completed Step 1 Actions purge and rebuild across all ${formulas.length} formulas.`);
