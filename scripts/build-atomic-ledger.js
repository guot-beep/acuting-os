...
function determineDispositionAndFields(drugId, srcItem, canonicalDrugs) {
  const norm = srcItem.normalized_text;
  const sec = srcItem.source_section;
  const drugRecord = canonicalDrugs.find(d => d.id === drugId);
  ...
  // Source URL items in v7 raw MD: check if exact URL matches canonical record's dailymed_url
  if (norm.startsWith('https://dailymed.nlm.nih.gov/') || norm.startsWith('https://www.dailymed.nlm.nih.gov/')) {
    if (drugRecord && drugRecord.dailymed_url && norm.includes(drugRecord.dailymed_setid)) {
      return { disposition: 'canonical', canonical_field: 'dailymed_url', staging_field: null };
    } else {
      // Old source-pack URL replaced by newer verified DailyMed label -> staging provenance!
      return { disposition: 'staging', canonical_field: null, staging_field: 'source_board_items' };
    }
  }
  ...
  if (drugId === 'drug.metoprolol') {
    ...
    if (norm.includes('Current label has a boxed warning against abrupt cessation')) return { disposition: 'staging', canonical_field: null, staging_field: 'source_board_items' }; // Moved to warnings in selected label!
...
function buildFullAtomicLedger() {
  const canonicalDrugs = JSON.parse(fs.readFileSync('data/pharmacology/drugs.json', 'utf8')).records;
  const ledger = [];
  ...
  all15DrugIds.forEach(drugId => {
    ...
    rawFacts.forEach(src => {
      ...
      const { disposition, canonical_field, staging_field } = determineDispositionAndFields(drugId, src, canonicalDrugs);
  ...
  const derivedCandidateFlags = [
    ...
    },
    {
      ...
      derived: true,
      reason: 'Inferred from abrupt cessation warning in source narrative.'
    }
  ];
  ...
  const stagingStore = {
    ...
    disposition_definitions: {
      ...
      staging: 'Primary disposition is preserved in staging data (unmapped board/clinical impact bullets or replaced source-pack URLs)',
